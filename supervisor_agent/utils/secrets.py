...
class SecretsManager:
    ...
    def _init_google_secret_manager(self):
        ...
        try:
            ...
            raise SecretProviderError(f"Failed to initialize Google Secret Manager: {e}")
    ...
    def _get_from_google_secret_manager(self, secret_name: str, version: str = "latest") -> Optional[str]:
        ...
        try:
            ...
            if value is None:
                if default is not None:
                    ...
                    raise SecretNotFoundError(f"Secret '{secret_name}' not found in {provider.value}")
                else:
                    self._log_access(secret_name, provider, False)
                    return None
    ...
    def set_secret(
        ...
        if provider == SecretProvider.GOOGLE_SECRET_MANAGER:
            ...
            raise SecretProviderError(f"Provider {provider.value} doesn't support setting secrets")
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if self.provider == SecretProvider.GOOGLE_SECRET_MANAGER:
                ...
                if self._gsm_client:
                    parent = f"projects/{self.project_id}"
                    self._gsm_client.list_secrets(request={"parent": parent, "page_size": 1})
...
def get_secret(secret_name: str, default: Optional[str] = None, required: bool = True) -> Optional[str]:
    ...
    return get_secrets_manager().get_secret(secret_name, default=default, required=required)
