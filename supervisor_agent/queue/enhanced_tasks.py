    ...
    try:
        ...
        if not agent:
            ...
            if quota_manager.are_all_agents_saturated(db):
                asyncio.run(notification_manager.send_quota_exhausted_alert(
        ...
        if not result.get("optimization_metadata", {}).get("was_cached", False):
            ...
            estimated_messages = quota_manager.estimate_messages_from_task(task.type, payload_size)
        ...
        if result["success"]:
            ...
            if task.retry_count >= settings.max_retries:
                ...
                asyncio.run(notification_manager.send_task_failure_alert(
                ...
                update_data = schemas.TaskUpdate(
                    status=models.TaskStatus.RETRY,
                    error_message=error_message
                )
                crud.TaskCRUD.update_task(db, task_id, update_data)
    ...
    try:
        ...
        batch_summary = {
            ...
            "optimization_metrics": {
                "cache_hit_rate": optimization_metrics["processing_stats"]["cache_hit_rate_percent"],
                "batch_efficiency": optimization_metrics["processing_stats"]["batch_efficiency_percent"],
        ...
        return {
            "batch_summary": batch_summary,
            "results": [{"task_id": task.id, "success": result["success"]} 
...
def get_optimization_metrics():
    ...
    try:
        ...
        asyncio.run(notify_quota_update({
            "type": "metrics_update",
            "metrics": metrics,
