    ...
    try:
        ...
        for batch in batches:
            ...
            if not task_batcher.can_batch_tasks(batch):
                logger.warning(f"Cannot batch {len(batch)} tasks, processing individually")
                for task in batch:
        ...
        logger.info(f"Processed {processed_batches} batches with {total_tasks} total tasks")
...
def health_check_task():
    ...
    try:
        ...
        return {"status": "healthy" if not health_issues else "degraded", "issues": health_issues}
