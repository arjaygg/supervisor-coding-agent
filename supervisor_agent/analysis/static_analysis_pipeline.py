...
class StaticAnalysisPipeline:
    ...
    def _run_parallel_analysis(
        self, repo_path: Path, include_file_details: bool, semgrep_timeout: int
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit both analyses
                scc_future = executor.submit(
                    self._run_scc_analysis, repo_path, include_file_details
                )
                semgrep_future = executor.submit(
                    self._run_semgrep_analysis, repo_path, semgrep_timeout
                )
    ...
    def _run_scc_analysis(
        self, repo_path: Path, include_file_details: bool
    ) -> SCCResult:
        """Run SCC analysis."""
        try:
            return self.scc_analyzer.analyze_repository(
                str(repo_path), include_files=include_file_details
            )
        except Exception as e:
    ...
    def _run_semgrep_analysis(
        ...
        try:
            return self.semgrep_analyzer.analyze_repository(
                str(repo_path), timeout_seconds=timeout
            )
        except Exception as e:
    ...
    def _generate_unified_insights(
        self, scc_result: SCCResult, semgrep_result: SemgrepResult
    ) -> UnifiedInsights:
        ...
        try:
            ...
            semgrep_insights = self.semgrep_analyzer.get_security_insights(
                semgrep_result
            )
            ...
            risk_profile = self._calculate_risk_profile(
                ...
                len(technical_debt),
                semgrep_result.summary["by_severity"]["ERROR"],
            )
    ...
    def _calculate_code_quality_score(
        self, scc_result: SCCResult, scc_insights: Dict[str, Any]
    ) -> int:
        ...
        try:
            ...
            high_complexity_langs = [
                lang
                for lang, data in complexity_dist.items()
        ...
        try:
            return {
                "codebase_summary": {
                    ...
                    "primary_languages": scc_result.repository_summary[
                        "primary_languages"
                    ],
                ...
                "actionable_insights": {
                    "immediate_actions": len(
                        [
                            r
                            for r in self._generate_priority_recommendations(
                                scc_result,
                                semgrep_result,
                                scc_insights,
                                semgrep_insights,
                    ...
                    "refactoring_candidates": (
                        len(
                            [
                                f
                                for f in scc_result.file_details
    ...
    def _calculate_risk_profile(
        ...
        try:
            ...
            if (
                ...
                or security_score < 60
                or code_quality_score < 40
                or technical_debt_count > 5
            ):
                return "HIGH"
            ...
            if (
                security_score < 80
                or code_quality_score < 70
                or maintainability_score < 60
                or technical_debt_count > 2
            ):
                return "MEDIUM"
