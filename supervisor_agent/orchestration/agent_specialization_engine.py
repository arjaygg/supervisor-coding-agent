...
from supervisor_agent.providers.base_provider import (
    AIProvider, ProviderCapabilities, TaskCapability, ProviderType
)
...
class SpecializationCapability:
    ...
    def matches_task(self, task_type: TaskCapability, context: Dict[str, Any] = None) -> bool:
...
class DefaultTaskAnalyzer(TaskAnalyzer):
    ...
    async def analyze_task_characteristics(self, task: Task) -> Dict[str, Any]:
        ...
        self.logger.debug("Task characteristics analyzed", 
                         task_id=task.id,
...
class DefaultCapabilityMatcher(CapabilityMatcher):
    ...
    async def match_capabilities(
        ...
        self.logger.debug("Capability matching completed",
                         task_type=task_type,
                         domain=domain,
...
class DefaultPerformancePredictor(PerformancePredictor):
    ...
    async def predict_performance(
        ...
        if task.type:
            task_history = specialty_history.get("task_types", {}).get(task.type.value, {})
            if task_history:
                ...
                for metric in predictions:
                    if metric in task_history:
                        ...
                        predictions["confidence"] = min(predictions["confidence"] + 0.2, 1.0)
        
        self.logger.debug("Performance prediction completed",
...
class DefaultSpecializationStrategy:
    ...
    async def calculate_specialization_score(
        ...
        reasoning = self._generate_reasoning(
            capability_match, provider_availability, 
            historical_performance, context_match
        )
            ...
            for provider_type in capability.provider_preferences:
                if provider_type in provider_status:
                    ...
                    if status.get("status") == "active":
                        ...
                        score = (1.0 - status.get("load", 0.0)) * capability.proficiency_score
                        if score > best_score:
                            best_score = score
...
class AgentSpecializationEngine:
    ...
    def __init__(
        ...
        self.performance_predictor = performance_predictor or DefaultPerformancePredictor()
        self.specialization_strategy = specialization_strategy or DefaultSpecializationStrategy()
    ...
    async def select_best_specialist(
        ...
        """
        self.logger.info("Starting agent specialization selection",
                        task_id=task.id,
                        task_type=task.type.value if task.type else "unknown")
        ...
        try:
            ...
            task_characteristics = await self.task_analyzer.analyze_task_characteristics(task)
            ...
            if not capability_matches:
                ...
                return await self.specialization_strategy.calculate_specialization_score(
                    AgentSpecialty.GENERAL_DEVELOPER, task, context,
                    self.agent_capabilities, provider_status
                )
            
            # Calculate specialization scores for top matches
            best_score = None
            for specialty, capability_match in capability_matches[:5]:  # Top 5 candidates
            ...
            self.logger.info("Agent specialization selection completed",
                           task_id=task.id,
            ...
            self.logger.error("Agent specialization selection failed",
                            task_id=task.id,
                            error=str(e))
    ...
    async def update_agent_capabilities(
        ...
        self.logger.info("Agent capabilities updated",
                        specialty=specialty.value,
    ...
    def _initialize_agent_capabilities(self) -> Dict[AgentSpecialty, List[SpecializationCapability]]:
        ...
        capabilities = {
            AgentSpecialty.CODE_ARCHITECT: [
                SpecializationCapability(
                    ...
                    task_types=[TaskCapability.FEATURE_DEVELOPMENT, TaskCapability.GENERAL_CODING],
                    provider_preferences=[ProviderType.CLAUDE_CLI, ProviderType.ANTHROPIC_API],
                    context_requirements={"complexity": ["complex", "moderate"], "domain": "architecture"},
                    performance_metrics={"success_rate": 0.9, "avg_execution_time": 600.0}
                ),
                SpecializationCapability(
                    name="Architecture Review",
                    proficiency_score=0.85,
                    task_types=[TaskCapability.CODE_REVIEW, TaskCapability.CODE_ANALYSIS],
                    provider_preferences=[ProviderType.CLAUDE_CLI],
                    context_requirements={"domain": "architecture"},
                    performance_metrics={"success_rate": 0.85, "avg_execution_time": 300.0}
                )
            ],
            ...
            AgentSpecialty.CODE_REVIEWER: [
                SpecializationCapability(
                    ...
                    performance_metrics={"success_rate": 0.9, "avg_execution_time": 240.0}
                )
            ],
            ...
            AgentSpecialty.BUG_HUNTER: [
                SpecializationCapability(
                    ...
                    performance_metrics={"success_rate": 0.8, "avg_execution_time": 360.0}
                )
            ],
            ...
            AgentSpecialty.PERFORMANCE_OPTIMIZER: [
                SpecializationCapability(
                    ...
                    performance_metrics={"success_rate": 0.75, "avg_execution_time": 480.0}
                )
            ],
            ...
            AgentSpecialty.SECURITY_ANALYST: [
                SpecializationCapability(
                    ...
                    performance_metrics={"success_rate": 0.9, "avg_execution_time": 420.0}
                )
            ],
            ...
            AgentSpecialty.DOCUMENTATION_WRITER: [
                SpecializationCapability(
                    ...
                    performance_metrics={"success_rate": 0.8, "avg_execution_time": 240.0}
                )
            ],
            ...
            AgentSpecialty.REFACTORING_SPECIALIST: [
                SpecializationCapability(
                    ...
                    task_types=[TaskCapability.REFACTORING, TaskCapability.CODE_ANALYSIS],
                    provider_preferences=[ProviderType.CLAUDE_CLI],
                    context_requirements={"domain": "refactoring"},
                    performance_metrics={"success_rate": 0.75, "avg_execution_time": 360.0}
                )
            ],
            ...
            AgentSpecialty.DOMAIN_EXPERT: [
                SpecializationCapability(
                    ...
                    proficiency_score=0.85,
                    task_types=[TaskCapability.FEATURE_DEVELOPMENT, TaskCapability.CODE_REVIEW],
                    provider_preferences=[ProviderType.CLAUDE_CLI, ProviderType.ANTHROPIC_API],
                    context_requirements={"complexity": ["complex"]},
                    performance_metrics={"success_rate": 0.85, "avg_execution_time": 450.0}
                )
            ],
            ...
            AgentSpecialty.GENERAL_DEVELOPER: [
                SpecializationCapability(
                    ...
                    provider_preferences=[ProviderType.CLAUDE_CLI, ProviderType.LOCAL_MOCK],
                    context_requirements={},
                    performance_metrics={"success_rate": 0.7, "avg_execution_time": 360.0}
                )
            ]
        }
