...
from supervisor_agent.providers.base_provider import AIProvider, ProviderType, TaskCapability
from supervisor_agent.core.multi_provider_service import MultiProviderService
from supervisor_agent.orchestration.agent_specialization_engine import AgentSpecializationEngine, AgentSpecialty
...
class MultiProviderCoordinator:
    ...
    async def _get_available_providers(self, task: CoordinationTask) -> List[ProviderType]:
    ...
    async def _supports_task_type(self, provider_type: ProviderType, task_type: TaskCapability) -> bool:
        """Check if provider supports task type."""
    ...
    def _order_failover_chain(self, providers: List[ProviderType]) -> List[ProviderType]:
        """Order providers for failover chain."""
        # Sort by availability score descending
        return sorted(
            providers,
            key=lambda p: self.provider_metrics.get(p, ProviderMetrics(p)).availability_score,
            reverse=True
        )
    ...
    async def _execute_parallel(
        ...
        tasks = [
            self._execute_single(task, provider)
            for provider in providers
        ]
    ...
    def _calculate_cost(self, provider_type: ProviderType, execution_time: float) -> float:
    ...
    def _update_provider_metrics(self, result: ExecutionResult):
        ...
        if metrics.total_requests == 1:
            ...
            metrics.avg_response_time = (
                (metrics.avg_response_time * (metrics.total_requests - 1) + result.execution_time)
        ...
        if result.cost > 0:
            if metrics.total_requests == 1:
                ...
                metrics.cost_per_request = (
                    (metrics.cost_per_request * (metrics.total_requests - 1) + result.cost)
                    / metrics.total_requests
                )
    ...
    def get_coordination_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_execution_time": sum(r.execution_time for r in self.execution_history) / total_executions if total_executions > 0 else 0,
