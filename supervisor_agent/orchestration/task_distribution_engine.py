...
from supervisor_agent.providers.base_provider import ProviderType, TaskCapability, AIProvider
...
from supervisor_agent.orchestration.multi_provider_coordinator import (
    MultiProviderCoordinator, CoordinationStrategy, ProviderMetrics
)
...
class IntelligentTaskSplitter:
    ...
    def _load_complexity_patterns(self) -> Dict[str, Any]:
        ...
        return {
            "keywords": {
                ...
                "complex": ["analyze", "optimize", "integrate", "coordinate", "orchestrate"],
                "highly_complex": ["architect", "design system", "full implementation", "end-to-end"]
            },
            "step_indicators": ["then", "after", "next", "followed by", "subsequently"],
            "dependency_indicators": ["depends on", "requires", "needs", "after completing"],
            "parallel_indicators": ["simultaneously", "in parallel", "concurrently", "at the same time"]
        }
    ...
    def analyze_task_complexity(self, task: Task) -> ComplexityAnalysis:
        ...
        try:
            ...
            analysis = ComplexityAnalysis(
                ...
                estimated_execution_time=self._estimate_execution_time(estimated_steps, complexity_level),
            ...
            self.logger.error("Error analyzing task complexity", task_id=task.id, error=str(e))
    ...
    def _extract_task_content(self, task: Task) -> str:
        ...
        if task.payload:
            if isinstance(task.payload, dict):
                ...
                for field in ["description", "prompt", "instructions", "content", "message"]:
    ...
    def _calculate_complexity_score(self, content: str) -> float:
        ...
        step_matches = sum(1 for indicator in self._complexity_patterns["step_indicators"] 
                          if indicator in content)
        score += step_matches * 0.2
        
        # Dependency indicator scoring
        dep_matches = sum(1 for indicator in self._complexity_patterns["dependency_indicators"] 
                         if indicator in content)
    ...
    def _estimate_steps(self, content: str) -> int:
        ...
        step_count = sum(1 for indicator in self._complexity_patterns["step_indicators"] 
                        if indicator in content)
    ...
    def _identify_dependencies(self, content: str) -> List[str]:
        ...
        common_dependencies = [
            "database", "api", "service", "component", "module", 
    ...
    def _recommend_splitting_strategy(self, complexity: TaskComplexity, 
                                    content: str, dependencies: List[str]) -> SplittingStrategy:
        ...
        parallel_indicators = sum(1 for indicator in self._complexity_patterns["parallel_indicators"] 
                                if indicator in content)
        ...
        linear_indicators = sum(1 for indicator in self._splitting_rules["linear_split_indicators"] 
                              if indicator in content)
        ...
        if complexity in [TaskComplexity.COMPLEX, TaskComplexity.HIGHLY_COMPLEX] and len(dependencies) > 3:
            return SplittingStrategy.HIERARCHICAL_SPLIT
        
        # Default to linear for moderate complexity
        return SplittingStrategy.LINEAR_SPLIT
    
    def _estimate_resource_requirements(self, complexity: TaskComplexity, steps: int) -> Dict[str, Any]:
        """Estimate resource requirements based on complexity."""
        base_requirements = {
            TaskComplexity.SIMPLE: {"cpu": "low", "memory": "low", "io": "low"},
            TaskComplexity.MODERATE: {"cpu": "medium", "memory": "medium", "io": "medium"},
            TaskComplexity.COMPLEX: {"cpu": "high", "memory": "high", "io": "medium"},
            TaskComplexity.HIGHLY_COMPLEX: {"cpu": "very_high", "memory": "very_high", "io": "high"}
        }
    ...
    def _calculate_confidence_score(self, complexity_score: float, 
                                  dependency_count: int, content: str) -> float:
    ...
    def _generate_reasoning(self, complexity: TaskComplexity, strategy: SplittingStrategy,
                          steps: int, dependencies: List[str]) -> str:
        ...
        if dependencies:
            reasoning_parts.append(f"Identified {len(dependencies)} dependencies: {', '.join(dependencies[:3])}")
...
class DependencyManager:
    ...
    def build_dependency_graph(self, tasks: List[TaskSplit]) -> DependencyGraph:
        ...
        total_time = sum(max(nodes[task_id].estimated_duration for task_id in level) 
    ...
    def _resolve_circular_dependencies(self, edges: Dict[str, List[str]]) -> Dict[str, List[str]]:
    ...
    def _calculate_execution_levels(self, tasks: List[TaskSplit], 
    ...
    def _identify_critical_path(self, tasks: List[TaskSplit], 
        ...
        while queue:
            current = queue.pop(0)
            current_distance = distances[current] + task_dict[current].estimated_duration
    ...
    def _calculate_parallelization_potential(self, execution_levels: List[List[str]], 
                                           tasks: List[TaskSplit]) -> float:
...
class TaskDistributionEngine:
    ...
    def __init__(self, 
        ...
        self.agent_specialization_engine = agent_specialization_engine or AgentSpecializationEngine()
        if multi_provider_coordinator is None:
            ...
            self.multi_provider_coordinator = MultiProviderCoordinator(provider_service=provider_service)
        else:
    ...
    async def distribute_task(
        ...
        try:
            ...
            task_splits = await self._generate_task_splits(task_obj, complexity_analysis)
            
            # Step 3: Build dependency graph
            dependency_graph = self.dependency_manager.build_dependency_graph(task_splits)
    ...
    async def _generate_task_splits(self, task: Task, 
                                  analysis: ComplexityAnalysis) -> List[TaskSplit]:
        ...
        self.logger.info(
            ...
            task_id=task.id,
            splits_count=len(task_splits)
        )
        
        return task_splits
    
    def _create_task_splits_by_strategy(self, task: Task, analysis: ComplexityAnalysis) -> List[TaskSplit]:
        ...
        if analysis.splitting_recommendation == SplittingStrategy.LINEAR_SPLIT:
            ...
            step_duration = analysis.estimated_execution_time // analysis.estimated_steps
            
            for i in range(analysis.estimated_steps):
                task_splits.append(TaskSplit(
                    split_id=f"{task.id}_{i}",
                    original_task_id=str(task.id),
                    subtask_index=i,
                    task_type=self._determine_subtask_type(i, analysis.estimated_steps),
                    agent_specialty=self._determine_agent_specialty(i, analysis.estimated_steps),
                    priority=task.priority or 5,
            ...
            for i in range(parallel_count):
                task_splits.append(TaskSplit(
                    ...
                    agent_specialty=self._determine_agent_specialty(i, parallel_count),
                    priority=task.priority or 5,
                    estimated_duration=step_duration,
                    parallelizable=True
                ))
    ...
    def _create_hierarchical_subtasks(self, task: Task, analysis: ComplexityAnalysis) -> List[TaskSplit]:
    ...
    async def _optimize_distribution_strategy(
        ...
        if dependency_graph.parallelization_potential > 0.7:
            if requested_strategy in [DistributionStrategy.PARALLEL, DistributionStrategy.DEPENDENCY_AWARE]:
        ...
        if complexity_analysis.complexity_level in [TaskComplexity.COMPLEX, TaskComplexity.HIGHLY_COMPLEX]:
    ...
    async def _assign_providers(
        ...
        try:
            available_providers = await self.multi_provider_coordinator.get_available_providers()
        except:
            available_providers = [ProviderType.CLAUDE_CLI]  # Fallback
        
        if preferred_providers:
            available_providers = [p for p in available_providers if p in preferred_providers]
        ...
        if strategy == DistributionStrategy.LOAD_BALANCED:
            ...
            for i, split in enumerate(task_splits):
                assignments[split.split_id] = available_providers[i % len(available_providers)]
        else:
            # Default assignment
    ...
    def _calculate_resource_allocation(self, task_splits: List[TaskSplit],
                                     provider_assignments: Dict[str, ProviderType]) -> Dict[str, Dict[str, Any]]:
        ...
        for split in task_splits:
            allocation[split.split_id] = {
                ...
                "memory_mb": min(2048, 512 * (2 if split.estimated_duration > 300 else 1)),
    ...
    def _estimate_execution_cost(self, task_splits: List[TaskSplit],
                               provider_assignments: Dict[str, ProviderType]) -> float:
    ...
    async def _validate_execution_plan(self, plan: ExecutionPlan) -> Dict[str, Any]:
        ...
        if plan.estimated_total_time > 3600:
            warnings.append(f"Long estimated execution time: {plan.estimated_total_time // 60} minutes")
...
def create_task_distribution_engine(
    ...
    if multi_provider_coordinator is None and multi_provider_service is not None:
        multi_provider_coordinator = MultiProviderCoordinator(provider_service=multi_provider_service)
