...
class DynamicTaskScheduler:
    ...
    async def _optimize_task_priority(self, task: ScheduledTask) -> TaskPriority:
        ...
        try:
            ...
            priority_value = optimization.get("recommended_priority", task.priority.value)
    ...
    async def _calculate_scheduling_score(self, task: ScheduledTask) -> float:
        ...
        if task.deadline:
            ...
            if time_to_deadline > 0:
                urgency_factor = max(0, 10 / time_to_deadline)  # More urgent = lower score
            else:
        ...
        final_score = priority_score + urgency_factor - resource_efficiency - dependency_factor
    ...
    async def _evaluate_immediate_scheduling(self):
        ...
        while self.task_queue and len(tasks_to_schedule) < 5:  # Limit immediate scheduling
            score, task = heapq.heappop(self.task_queue)
            
            # Check if task can be scheduled now
            if await self._can_schedule_task(task, available_resources, available_agents):
        ...
        for req in task.resource_requirements:
            if req.is_required:
                if self.resource_pool[req.resource_type] >= req.amount:
                    ...
                    raise RuntimeError(f"Insufficient {req.resource_type.value} resources")
    ...
    async def _simulate_task_execution(self, task: ScheduledTask, allocation: ResourceAllocation):
    ...
    async def _optimize_scheduling(self):
        ...
        optimization = await self.ai_optimizer.optimize_schedule(self.task_queue, context)
    ...
    async def _reorder_task_queue(self, new_priorities: Dict[str, float]):
        ...
        for task in tasks:
            new_score = new_priorities.get(task.task_id, await self._calculate_scheduling_score(task))
    ...
    def _calculate_current_load(self) -> Dict[str, float]:
        ...
        return {
            ...
            "resource_utilization": sum(
                ...
                for total, available in [(100, self.resource_pool[rt]) for rt in ResourceType]
            ) / len(ResourceType),
            "agent_utilization": len(self.running_tasks) / max(len(self.agent_capacities), 1)
        }
    ...
    def _get_system_constraints(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_concurrent_tasks": sum(cap.max_concurrent_tasks for cap in self.agent_capacities.values()),
    ...
    async def cancel_task(self, task_id: str) -> bool:
        ...
        if task_id in self.running_tasks:
            ...
            # Release resources
            allocation = next(
                (alloc for alloc in self.allocated_resources.values() if alloc.task_id == task_id),
                None
            )
            if allocation:
                await self._release_resources(allocation)
...
class AdaptiveSchedulingController:
    ...
    def should_adapt_strategy(self, current_metrics: Dict[str, Any]) -> bool:
        ...
        if queue_utilization > 0.8 and self.current_strategy != SchedulingStrategy.AI_OPTIMIZED:
            return True
...
async def create_dynamic_task_scheduler(claude_api_key: Optional[str] = None) -> DynamicTaskScheduler:
