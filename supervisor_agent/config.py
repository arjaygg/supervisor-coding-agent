...
class Settings(BaseSettings):
    ...
    enable_multi_provider: bool = Field(
        default=False, 
        description="Enable multi-provider functionality"
    )
        ...
        if self.enable_multi_provider:
            try:
                ...
                if not provider_configs:
                    warnings.append("Multi-provider enabled but no providers configured")
                else:
                    enabled_providers = [p for p in provider_configs if p.get("enabled", True)]
                    if not enabled_providers:
                        warnings.append("Multi-provider enabled but no providers are enabled")
                    else:
                        warnings.append(f"Multi-provider enabled with {len(enabled_providers)} provider(s)")
                
                # Validate load balancing strategy
                valid_strategies = ["round_robin", "least_loaded", "fastest_response", "priority_based", "capability_based"]
                if self.default_load_balancing_strategy not in valid_strategies:
                    warnings.append(f"Invalid load balancing strategy: {self.default_load_balancing_strategy}")
        ...
        if self.enable_multi_provider:
            try:
                ...
                info.update({
                    ...
                    "provider_types": list(set(p.get("type") for p in enabled_providers))
                })
            except Exception:
                info.update({
                    "multi_provider_enabled": True,
