    ...
    async def cleanup_expired_entries(self):
        ...
        async with self._lock:
            ...
            for client_id, entry in self.clients.items():
                if (now - entry.window_start > self.window_duration * 2 and
                    now - entry.burst_window_start > self.burst_window * 2):
...
class SecurityRateLimiter:
        ...
        if not allowed:
            ...
            if ip not in self.suspicious_ips:
                ...
                if time.time() - self.suspicious_ips[ip] < 300:  # 5 minutes
                    ...
                    logger.warning(f"IP blocked for repeated rate limit violations: {ip}")
    ...
    async def cleanup(self):
        ...
        now = time.time()
        expired_suspicious = [
            ip for ip, timestamp in self.suspicious_ips.items()
        ...
        expired_blocked = [
            ip for ip, timestamp in self.blocked_ips.items()
            if now - timestamp > 3600  # 1 hour
        ]
