...
class ClaudeCodeSubscriptionMonitor:
    ...
    async def check_claude_code_status(self) -> ClaudeCodeUsage:
        ...
        try:
            ...
            if result["success"]:
                ...
                is_limit_error = any(
                    pattern in error_msg for pattern in self.known_limit_errors
                )
                ...
                if is_limit_error:
                    ...
                    usage = ClaudeCodeUsage(
                        ...
                        last_successful_request=(
                            self.current_usage.last_successful_request
                            if self.current_usage
                            else None
                        ),
                        consecutive_failures=(
                            (self.current_usage.consecutive_failures + 1)
                            if self.current_usage
                            else 1
                        ),
                    ...
                    usage = ClaudeCodeUsage(
                        ...
                        requests_limit=self._estimate_requests_limit(),
                        usage_percentage=(
                            self.current_usage.usage_percentage
                            if self.current_usage
                            else 50.0
                        ),
                        reset_time=self._estimate_reset_time(),
    ...
    async def should_defer_work(
        self, work_type: str, priority: int = 3
    ) -> Tuple[bool, str]:
        ...
        if not self.current_usage.is_available:
            return (
                True,
                f"Claude Code subscription unavailable ({self.current_usage.usage_percentage:.1f}% used)",
            )
        ...
        if self.current_usage.usage_percentage >= self.emergency_threshold:
            return (
                True,
                f"Emergency threshold reached ({self.current_usage.usage_percentage:.1f}%)",
            )
        ...
        if self.current_usage.consecutive_failures >= 3:
            return (
                True,
                f"Multiple consecutive failures ({self.current_usage.consecutive_failures})",
            )
        ...
        if self.current_usage.usage_percentage >= self.warning_threshold:
            if priority > 2:  # Only high/medium priority allowed
                return (
                    True,
                    f"Low priority work deferred at {self.current_usage.usage_percentage:.1f}% usage",
                )
        ...
        try:
            ...
            estimated_usage_increase = (
                prediction.predicted_value / 100
            )  # Convert to percentage
            projected_usage = (
                self.current_usage.usage_percentage + estimated_usage_increase
            )

            if projected_usage >= self.emergency_threshold:
                return (
                    True,
                    f"Work would push usage to {projected_usage:.1f}% (over {self.emergency_threshold}% threshold)",
                )
    ...
    async def defer_work(
        ...
        try:
            ...
            if len(self.pending_work) >= self.max_pending_work_items:
                logger.warning(
                    f"Max pending work items reached ({self.max_pending_work_items})"
                )
                # Remove oldest low-priority items to make space
                self.pending_work = [
                    w for w in self.pending_work if w.priority <= 2
                ] + self.pending_work[-self.max_pending_work_items // 2 :]
            ...
            pending_work = PendingWork(
                ...
                retry_count=0,
                reason_deferred=(
                    f"Claude Code usage at {self.current_usage.usage_percentage:.1f}%"
                    if self.current_usage
                    else "Subscription limit"
                ),
            )
    ...
    async def resume_pending_work(self) -> int:
        ...
        estimated_work_capacity = max(
            1, int(available_capacity_pct / 10)
            ...
            try:
                ...
                if success:
                    ...
                    if work.retry_count >= work.max_retries:
                        logger.warning(
                            f"Work {work.work_id} exceeded max retries, removing"
                        )
                        self.pending_work.remove(work)
                    else:
                        # Schedule next retry with backoff
                        work.retry_after = current_time + timedelta(
                            minutes=work.retry_count * 10
                        )
                        logger.info(
                            f"Work {work.work_id} failed to resume, retry {work.retry_count}/{work.max_retries}"
                        )
    ...
    async def get_subscription_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "last_check": (
                self.last_limit_check.isoformat() if self.last_limit_check else None
            ),
        ...
        if self.current_usage:
            status["subscription_usage"] = {
                ...
                "reset_time": (
                    self.current_usage.reset_time.isoformat()
                    if self.current_usage.reset_time
                    else None
                ),
                "time_to_reset_hours": self.current_usage.time_to_reset_hours,
                "consecutive_failures": self.current_usage.consecutive_failures,
                "last_successful_request": (
                    self.current_usage.last_successful_request.isoformat()
                    if self.current_usage.last_successful_request
                    else None
                ),
            }
        ...
        if self.pending_work:
            ...
            status["pending_work_summary"] = {
                ...
                "by_type": by_type,
                "oldest_item": min(
                    self.pending_work, key=lambda w: w.created_at
                ).created_at.isoformat(),
                "next_retry": min(
                    self.pending_work, key=lambda w: w.retry_after
        ...
        try:
            if self.current_usage and self.current_usage.is_available:
                prediction = await self.usage_predictor.predict_usage(
                    provider_id="claude_code", horizon_minutes=120  # 2 hour prediction
                )
    ...
    def _schedule_periodic_tasks(self) -> None:
        ...
        monitor_claude_code_subscription.apply_async(
            countdown=self.check_interval_seconds
        )
    ...
    def _schedule_work_retry(self, work_id: str, retry_time: datetime) -> None:
        ...
        # Schedule retry task
        retry_claude_code_work.apply_async(args=[work_id], countdown=countdown)
    ...
    async def _resume_work_item(self, work: PendingWork) -> bool:
        ...
        try:
            ...
            should_defer, reason = await self.should_defer_work(
                work.work_type, work.priority
            )
            if should_defer:
                logger.debug(
                    f"Work {work.work_id} still needs to be deferred: {reason}"
                )
                return False
    ...
    async def _cleanup_old_work(self) -> None:
        ...
        self.pending_work = [
            work for work in self.pending_work if work.created_at > cutoff_time
        ]
