...
class TokenEstimator:
    ...
    def estimate_cost(
        ...
        """
        pricing = TokenEstimator.CLAUDE_PRICING.get(
            model, TokenEstimator.CLAUDE_PRICING["default"]
        )
...
class CostTracker:
    ...
    def track_task_execution(
    ...
    ) -> schemas.CostTrackingEntryResponse:
        """
        ...
        try:
            ...
            logger.info(
                f"Tracked cost for task {task_id}: ${estimated_cost} "
            ...
            self._update_usage_metrics(
                db, agent_id, 0, 0.0, execution_time_ms, success=False
            )
            raise
    ...
    def _update_usage_metrics(
        ...
        try:
            ...
            self._increment_metrics(
                db, daily_metrics, tokens, cost, execution_time_ms, success
            )
            ...
            self._increment_metrics(
                db, agent_metrics, tokens, cost, execution_time_ms, success
            )
    ...
    def _get_or_create_metrics(
        self, db: Session, metric_type: str, metric_key: str
    ) -> schemas.UsageMetricsResponse:
        """Get existing metrics or create new ones"""
        existing = (
            db.query(crud.models.UsageMetrics)
            .filter(
    ...
    def _increment_metrics(
        self,
        db: Session,
        metrics: schemas.UsageMetricsResponse,
        tokens: int,
        cost: float,
        execution_time_ms: int,
        success: bool,
