...
class CircuitBreakerState:
    ...
    def should_allow_request(self) -> bool:
        ...
        if self.state == "closed":
            ...
            if self.next_retry_time and datetime.now(timezone.utc) >= self.next_retry_time:
    ...
    def record_failure(self) -> None:
        ...
        if self.failure_count >= self.failure_threshold:
            self.state = "open"
            self.next_retry_time = datetime.now(timezone.utc) + timedelta(seconds=self.timeout_seconds)
...
class RecoveryOrchestrator:
        ...
        try:
            ...
            if provider_id == "claude_code":
                ...
                health = ProviderHealth(
                    ...
                    last_failure=datetime.now(timezone.utc) if claude_status.consecutive_failures > 0 else None,
                    consecutive_failures=claude_status.consecutive_failures,
                    consecutive_successes=0 if claude_status.consecutive_failures > 0 else 1,
                    success_rate_1h=max(0.0, 100.0 - (claude_status.consecutive_failures * 20)),
    ...
    async def attempt_recovery(self, provider_id: str, recovery_reason: str) -> bool:
        ...
        try:
            ...
            if health.status == ProviderStatus.HEALTHY:
                logger.info(f"Provider {provider_id} already healthy, no recovery needed")
                return True
            
            logger.info(f"Attempting recovery for provider {provider_id}: {recovery_reason}")
    ...
    async def perform_failover(
        ...
        try:
            ...
            alternative_providers = await self._get_alternative_providers(failed_provider_id)
            
            if not alternative_providers:
                logger.warning(f"No alternative providers available for failover from {failed_provider_id}")
                return None
            
            # Select best provider based on health scores
            best_provider = max(alternative_providers, key=lambda p: self.provider_health.get(p, ProviderHealth(
                provider_id=p, status=ProviderStatus.DEGRADED, last_success=None, last_failure=None,
                consecutive_failures=0, consecutive_successes=0, success_rate_1h=50.0, response_time_avg=5.0,
                quota_available=True, quota_reset_time=None, error_patterns=[], recovery_attempts=0
            )).health_score)
            
            # Check if target provider can handle the load
            can_handle = await self._check_provider_capacity(best_provider, task_ids)
            if not can_handle:
                logger.warning(f"Alternative provider {best_provider} cannot handle failover load")
                return None
            
            # Perform the failover
            await self._execute_failover(failed_provider_id, best_provider, task_ids, reason)
            
            self.recovery_stats["failovers_performed"] += 1
            
            logger.info(f"Successfully failed over from {failed_provider_id} to {best_provider} for {len(task_ids)} tasks")
    ...
    async def get_recovery_status(self) -> Dict[str, Any]:
        ...
        for provider_id, health in self.provider_health.items():
            status["provider_health"][provider_id] = {
                ...
                "quota_reset_time": health.quota_reset_time.isoformat() if health.quota_reset_time else None,
            }
        ...
        for provider_id, cb in self.circuit_breakers.items():
            status["circuit_breakers"][provider_id] = {
                ...
                "next_retry_time": cb.next_retry_time.isoformat() if cb.next_retry_time else None,
            }
    ...
    async def _update_circuit_breaker(self, provider_id: str, health: ProviderHealth) -> None:
    ...
    async def _update_system_state(self) -> None:
        ...
        if new_state != self.current_state:
            logger.info(f"System state changed from {self.current_state} to {new_state}")
        ...
        for provider_id, health in self.provider_health.items():
            if health.status in [ProviderStatus.FAILING, ProviderStatus.UNAVAILABLE]:
                ...
                if health.last_failure:
                    time_since_failure = datetime.now(timezone.utc) - health.last_failure
                    if time_since_failure > timedelta(minutes=5):  # Wait 5 minutes
                        if health.recovery_attempts < self.max_recovery_attempts:
                            await self.attempt_recovery(provider_id, "Automatic recovery")
    ...
    async def _process_pending_recovery_actions(self) -> None:
        ...
        ready_actions = [
            action for action in self.pending_recovery_actions
        ...
        for action in ready_actions:
            try:
                ...
                if action.attempt_count >= action.max_attempts:
                    ...
                    action.scheduled_time = current_time + timedelta(minutes=backoff_minutes)
    
    async def _check_quota_refresh_opportunities(self) -> None:
        """Check for quota refresh opportunities to resume work"""
        for provider_id, health in self.provider_health.items():
            if health.quota_reset_time and health.quota_reset_time <= datetime.now(timezone.utc):
                ...
                if provider_id == "claude_code":
                    ...
                    if resumed > 0:
                        logger.info(f"Resumed {resumed} tasks after quota refresh for {provider_id}")
    
    async def _attempt_provider_restart(self, provider_id: str, health: ProviderHealth) -> bool:
    ...
    async def _attempt_failure_recovery(self, provider_id: str, health: ProviderHealth) -> bool:
    ...
    async def _attempt_performance_recovery(self, provider_id: str, health: ProviderHealth) -> bool:
    ...
    async def _check_provider_capacity(self, provider_id: str, task_ids: List[int]) -> bool:
    ...
    async def _execute_failover(
        ...
        for task_id in task_ids:
            await self.schedule_recovery_retry(target_provider, [task_id], 1, f"Failover: {reason}")
    
    async def _execute_recovery_action(self, action: RecoveryAction) -> None:
        """Execute a specific recovery action"""
        logger.info(f"Executing recovery action: {action.action_type} for provider {action.provider_id}")
        ...
        if action.action_type == "retry":
            ...
            # Perform failover
            await self.perform_failover(action.provider_id, action.task_ids, action.reason)
            ...
            if action.provider_id in self.circuit_breakers:
                ...
                self.circuit_breakers[action.provider_id].next_retry_time = (
                    datetime.now(timezone.utc) + timedelta(seconds=self.circuit_breaker_timeout)
                )
...
def monitor_recovery_health():
    ...
    # Schedule next check
    monitor_recovery_health.apply_async(countdown=recovery_orchestrator.health_check_interval)
...
def process_recovery_actions():
    ...
    # Schedule next check
    process_recovery_actions.apply_async(countdown=recovery_orchestrator.recovery_check_interval)
...
def schedule_recovery_retry(provider_id: str, task_ids: List[int], reason: str):
    ...
    try:
        async def retry():
            await recovery_orchestrator.schedule_recovery_retry(provider_id, task_ids, 5, reason)
