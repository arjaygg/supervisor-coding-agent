...
class IntelligentTaskScheduler:
    ...
    def __init__(
        ...
        self.batch_candidates: Dict[str, List[int]] = defaultdict(list)  # task_type -> task_ids
    ...
    async def rebalance_queues(self) -> None:
        ...
        try:
            ...
            needs_rebalancing = (
                not claude_status.is_available or
                claude_status.usage_percentage > self.quota_warning_threshold
            )
            ...
            for priority in [TaskPriority.LOW, TaskPriority.MEDIUM]:
                ...
                while queue:
                    ...
                    heapq.heappush(
                        ...
                        (self._calculate_scheduling_score(metadata), timestamp, metadata)
                    )
    ...
    async def get_scheduling_status(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "last_scheduling_run": self.last_scheduling_run.isoformat() if self.last_scheduling_run else None,
                "queue_sizes": {},
                "batch_candidates": {},
    ...
    async def _get_scheduling_context(self) -> SchedulingContext:
        ...
        try:
            ...
            total_queue_size = sum(len(queue) for queue in self.priority_queues.values())
            ...
            if self.scheduling_stats["tasks_scheduled"] > 0:
                ...
                avg_processing_time = self.scheduling_stats.get("average_queue_time", 5.0)
            ...
            try:
                quota_status = await quota_monitor.check_quota_status("claude-cli-primary")
                if quota_status:
                    health_score = 1.0 - (quota_status["claude-cli-primary"].usage_percentage / 100.0)
    ...
    async def _apply_scheduling_strategy(self, context: SchedulingContext) -> List[SchedulingDecision]:
        ...
        try:
            ...
            logger.error(f"Error applying scheduling strategy {self.current_strategy}: {e}")
        
        return decisions
    
    async def _adaptive_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
    ...
    async def _emergency_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
        ...
        if critical_queue:
            ...
            decision = SchedulingDecision(
                ...
                estimated_completion=datetime.now(timezone.utc) + timedelta(minutes=metadata.estimated_duration_minutes),
    ...
    async def _conservative_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
    ...
    async def _normal_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
        ...
        for priority in [TaskPriority.CRITICAL, TaskPriority.HIGH, TaskPriority.MEDIUM]:
            ...
            while queue and scheduled_count < 3:  # Limit per priority
                ...
                if await self._check_dependencies_satisfied(metadata):
                    ...
                    heapq.heappush(queue, (self._calculate_scheduling_score(metadata), timestamp, metadata))
        
        return decisions
    
    async def _try_batch_scheduling(self, context: SchedulingContext) -> Optional[SchedulingDecision]:
            ...
            if decision.action == "schedule":
                ...
                for task_id in decision.task_ids:
                    ...
                    if task_id in self.task_metadata:
                        self.task_metadata[task_id].scheduled_at = decision.scheduled_time
                ...
                for task_id in decision.task_ids:
                    await claude_code_monitor.defer_work(
                        ...
                        priority=self.task_metadata[task_id].priority.value if task_id in self.task_metadata else 3
                    )
            
            logger.info(f"Executed scheduling decision: {decision.action} for tasks {decision.task_ids}")
            return True
    ...
    def _calculate_scheduling_score(self, metadata: TaskMetadata) -> float:
        ...
        if metadata.deadline:
            time_to_deadline = (metadata.deadline - datetime.now(timezone.utc)).total_seconds() / 3600
        ...
        age_hours = (datetime.now(timezone.utc) - metadata.created_at).total_seconds() / 3600
    ...
    async def _check_dependencies_satisfied(self, metadata: TaskMetadata) -> bool:
        ...
        try:
            ...
            logger.warning(f"Error checking dependencies for task {metadata.task_id}: {e}")
    ...
    def _update_scheduling_stats(self, decisions: List[SchedulingDecision]) -> None:
        ...
        if total_decisions > 0:
            efficiency = (self.scheduling_stats["tasks_scheduled"] + self.scheduling_stats["tasks_batched"]) / total_decisions
            self.scheduling_stats["scheduling_efficiency"] = efficiency
    
    # Additional strategy implementations
    async def _priority_first_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
        """Strict priority-based scheduling"""
        decisions = []
        # Implementation similar to normal scheduling but purely priority-based
        return decisions
    
    async def _quota_aware_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
        """Quota-conscious scheduling"""
        decisions = []
        # Implementation that heavily weights quota availability
        return decisions
    
    async def _cost_optimized_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
        """Cost-optimized scheduling"""
        decisions = []
        # Implementation that minimizes costs
        return decisions
    
    async def _performance_optimized_scheduling(self, context: SchedulingContext) -> List[SchedulingDecision]:
