...
from supervisor_agent.providers.base_provider import (
    ...
    ProviderResponse, CostEstimate, ProviderError
)
...
class TaskAffinityTracker:
    ...
    def __init__(self):
        self.task_relationships: Dict[str, List[str]] = {}  # task_id -> related_task_ids
        self.provider_assignments: Dict[str, str] = {}     # task_id -> provider_id
        self.provider_performance: Dict[str, List[float]] = {}  # provider_id -> success_rates
    ...
    def record_performance(self, provider_id: str, success: bool, execution_time: float) -> None:
        ...
        if len(self.provider_performance[provider_id]) > 100:
            self.provider_performance[provider_id] = self.provider_performance[provider_id][-100:]
...
class ProviderCoordinator:
    ...
    def __init__(
        ...
        logger.info(f"Provider Coordinator initialized with strategy: {strategy}, affinity: {affinity_strategy}")
    ...
    async def select_provider(self, task: Task, context: ExecutionContext) -> str:
        ...
        try:
            ...
            logger.debug(f"Available providers after context filtering: {available_providers}")
            
            # Step 2: Filter by task capabilities
            capable_providers = await self._filter_by_capabilities(task, available_providers)
            if not capable_providers:
                raise ProviderError(f"No providers capable of handling task type: {task.type}")
            ...
            if context.max_cost_usd:
                cost_viable_providers = await self._filter_by_cost(task, healthy_providers, context.max_cost_usd)
                if cost_viable_providers:
                    healthy_providers = cost_viable_providers
                else:
                    logger.warning(f"No providers within cost limit ${context.max_cost_usd}, proceeding with all healthy providers")
            
            # Step 5: Apply task affinity if enabled
            affinity_provider = None
            if self.affinity_strategy != TaskAffinityStrategy.NONE:
                affinity_provider = await self._check_task_affinity(task, healthy_providers)
                if affinity_provider:
                    logger.info(f"Selected provider {affinity_provider} based on task affinity")
                    await self._record_selection(task, affinity_provider, context, "affinity")
                    return affinity_provider
                    
            # Step 6: Apply load balancing strategy
            selected_provider = await self._apply_load_balancing_strategy(task, healthy_providers, context)
            
            if not selected_provider:
                raise ProviderError("Load balancing strategy failed to select a provider")
                
            # Record the selection for future affinity tracking
            await self._record_selection(task, selected_provider, context, str(self.strategy))
    ...
    async def select_backup_provider(self, task: Task, failed_provider_id: str, context: ExecutionContext) -> str:
        """Select a backup provider when the primary provider fails"""
        logger.warning(f"Selecting backup provider for task {task.id}, failed provider: {failed_provider_id}")
        ...
        try:
            ...
            logger.info(f"Selected backup provider {backup_provider} for task {task.id}")
            return backup_provider
        finally:
            # Restore original excluded providers
            context.exclude_providers = original_excluded
            
    async def get_provider_recommendations(self, task: Task, context: ExecutionContext) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for provider_id in capable_providers:
                ...
                performance_score = self.task_affinity_tracker.get_provider_score(provider_id)
                ...
                recommendation = {
                    ...
                    "recommendation_score": self._calculate_recommendation_score(
                        health.score, performance_score, float(cost_estimate.total_cost_usd)
                    )
                }
    ...
    def _calculate_recommendation_score(self, health_score: float, performance_score: float, cost_usd: float) -> float:
    ...
    async def _filter_by_context(self, context: ExecutionContext) -> List[str]:
        ...
        if context.exclude_providers:
            all_providers = [p for p in all_providers if p not in context.exclude_providers]
            
        return all_providers
        
    async def _filter_by_capabilities(self, task: Task, provider_ids: List[str]) -> List[str]:
    ...
    async def _filter_by_cost(self, task: Task, provider_ids: List[str], max_cost_usd: float) -> List[str]:
    ...
    async def _check_task_affinity(self, task: Task, provider_ids: List[str]) -> Optional[str]:
    ...
    async def _priority_based_selection(self, provider_ids: List[str]) -> str:
        ...
        for provider_id in provider_ids:
            ...
            provider_priorities[provider_id] = self.task_affinity_tracker.get_provider_score(provider_id)
            
        # Select provider with highest priority
        return max(provider_priorities.items(), key=lambda x: x[1])[0]
        
    async def _capability_based_selection(self, task: Task, provider_ids: List[str]) -> str:
    ...
    async def _is_provider_healthy(self, provider_id: str) -> bool:
        ...
        try:
            ...
            is_healthy = health.status in ["healthy", "degraded"]  # Allow degraded providers
    ...
    async def _record_selection(self, task: Task, provider_id: str, context: ExecutionContext, reason: str) -> None:
        ...
        logger.info(f"Provider selection recorded - Task: {task.id}, Provider: {provider_id}, Reason: {reason}")
