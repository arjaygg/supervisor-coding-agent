...
from supervisor_agent.db.models import Task, Provider
...
class RealTimeResourceMonitor:
        ...
        for resource_type, metrics in usage.metrics.items():
            ...
            # Check warning threshold
                ...
                alerts.append(ResourceAlert(
                    ...
                    current_value=utilization,
                    threshold_value=self._alert_thresholds["warning"][resource_type],
                    suggested_actions=[
                        "Consider scaling up",
                        "Monitor closely",
    ...
    async def track_provider_capacity(self, provider_id: str) -> Dict[str, Any]:
        ...
        # Analyze trends
        cpu_trend = self._calculate_trend([h.metrics[ResourceType.CPU_CORES].utilization_percent for h in history])
        memory_trend = self._calculate_trend([h.metrics[ResourceType.MEMORY_MB].utilization_percent for h in history])
        ...
        return {
            ...
            "time_span_minutes": (history[-1].last_updated - history[0].last_updated).total_seconds() / 60
        }
    ...
    async def detect_resource_bottlenecks(self, time_window_minutes: int = 60) -> List[Dict[str, Any]]:
        ...
        for provider_id, history in self._usage_history.items():
            recent_data = [
                ...
                if (datetime.now(timezone.utc) - usage.last_updated).total_seconds() <= time_window_minutes * 60
            ]
            ...
            for resource_type in ResourceType:
                utilizations = [
                    usage.metrics.get(resource_type, ResourceMetrics(
                        resource_type=resource_type,
                        total_capacity=0,
                        allocated=0,
                        reserved=0,
                ...
                if avg_utilization > 80 or max_utilization > 95:
                    bottlenecks.append({
                        ...
                        "recommendation": self._get_bottleneck_recommendation(resource_type, avg_utilization)
                    })
        
        return bottlenecks
    
    def _get_bottleneck_recommendation(self, resource_type: ResourceType, utilization: float) -> str:
        ...
        if resource_type == ResourceType.CPU_CORES:
            ...
            if utilization > 90:
                return "Increase memory allocation or optimize memory-intensive operations"
            else:
    ...
    async def generate_capacity_alerts(self, thresholds: Dict[str, float]) -> List[ResourceAlert]:
        ...
        for provider_id, history in self._usage_history.items():
            ...
            for resource_type, metrics in latest_usage.metrics.items():
                ...
                if threshold_key in thresholds and utilization >= thresholds[threshold_key]:
                    alerts.append(ResourceAlert(
                        alert_id=str(uuid.uuid4()),
...
class DynamicResourceAllocator:
    ...
    async def predict_resource_demand(self, time_horizon: int) -> DemandPrediction:
        ...
        for resource_type in ResourceType:
            ...
            ]) if current_usage else 0
            
            # Apply trend factor
            trend_factor = self._calculate_demand_trend_factor(resource_type, time_horizon)
            ...
            predicted_demand[resource_type] = max(0, min(100, predicted_value))  # Clamp to 0-100%
        ...
        confidence_score = self._calculate_prediction_confidence(historical_data, time_horizon)
    ...
    def _calculate_demand_trend_factor(self, resource_type: ResourceType, time_horizon: int) -> float:
        ...
        if resource_type == ResourceType.CPU_CORES:
            ...
            return base_factor + 0.1 * math.sin(time_horizon * math.pi / 720)  # 24-hour cycle
        elif resource_type == ResourceType.MEMORY_MB:
    ...
    def _calculate_prediction_confidence(self, historical_data: List[Dict[str, Any]], 
    ...
    async def optimize_allocation_strategy(self, current_usage: Dict[str, ResourceUsage]) -> AllocationOptimization:
        ...
        for provider_id, usage in current_usage.items():
            provider_recommendations = self._analyze_provider_optimization(provider_id, usage)
        ...
        confidence = min(1.0, len(recommendations) / 10)  # More recommendations = higher confidence
    ...
    def _analyze_provider_optimization(self, provider_id: str, usage: ResourceUsage) -> List[Dict[str, Any]]:
        ...
        for resource_type, metrics in usage.metrics.items():
            ...
            if utilization < 30:
                recommendations.append({
                    ...
                    "estimated_savings": metrics.allocated * 0.3 * 0.01,  # 30% savings estimate
                    "priority": "medium"
                })
    ...
    async def implement_cost_optimization(self, allocation: ResourceAllocation) -> Dict[str, Any]:
        """Implement cost optimization for a specific allocation."""
        self.logger.info("Implementing cost optimization", allocation_id=allocation.allocation_id)
        ...
        if optimization_actions:
            optimized_resources = {}
            for resource_type, amount in allocation.resources.items():
                action = next((a for a in optimization_actions if a["resource_type"] == resource_type.value), None)
                if action:
            ...
            allocation.metadata["optimization_date"] = datetime.now(timezone.utc).isoformat()
    ...
    def _calculate_resource_optimization(self, resource_type: ResourceType, current_amount: float) -> Dict[str, Any]:
        ...
        return {
            "optimized_amount": max(optimized_amount, current_amount * 0.5),  # Don't reduce by more than 50%
            "potential_savings": savings,
            "action": action
        }
    
    async def scale_resources_dynamically(self, demand: DemandPrediction) -> List[ScalingAction]:
        """Generate dynamic scaling actions based on demand prediction."""
        self.logger.info("Generating scaling actions", prediction_id=demand.prediction_id)
    ...
    def _generate_provider_scaling_actions(self, provider_id: str, usage: ResourceUsage, 
                                         demand: DemandPrediction) -> List[ScalingAction]:
        """Generate scaling actions for a specific provider."""
        actions = []
        
        for resource_type, metrics in usage.metrics.items():
            current_utilization = metrics.utilization_percent
            predicted_demand = demand.predicted_demand.get(resource_type, current_utilization)
    ...
    def _determine_scaling_need(self, current_utilization: float, predicted_demand: float, 
        ...
        elif predicted_demand < utilization_threshold_low and current_utilization < utilization_threshold_low:
            # Scale down possible
    ...
    async def allocate_resources(self, task_id: str, provider_id: str, 
                               resources: Dict[ResourceType, float],
                               priority: int = 5,
                               deadline: Optional[datetime] = None) -> ResourceAllocation:
    ...
    def _calculate_allocation_cost(self, provider_id: str, resources: Dict[ResourceType, float]) -> float:
    ...
    async def deallocate_resources(self, allocation_id: str) -> bool:
        ...
        duration_hours = (datetime.now(timezone.utc) - allocation.allocated_at).total_seconds() / 3600
