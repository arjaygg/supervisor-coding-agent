...
class DynamicResourceAllocator:
    ...
    def __init__(self):
        ...
        self.demand_predictions: Dict[int, ResourceDemand] = {}  # time_horizon -> prediction
    ...
    async def predict_resource_demand(self, time_horizon: int) -> ResourceDemand:
        ...
        prediction_multiplier = 1.0 + (time_horizon / 60) * 0.1  # Slight increase over time
        ...
        predicted_demand = ResourceDemand(
            cpu_demand=min(95.0, (current.cpu_percent + cpu_trend * time_horizon) * prediction_multiplier * time_multiplier),
            memory_demand=min(95.0, (current.memory_percent + memory_trend * time_horizon) * prediction_multiplier),
            disk_demand=min(95.0, current.disk_percent + disk_trend * time_horizon),
            network_demand=current.network_io * prediction_multiplier,
            confidence=min(0.9, 0.6 + len(recent_metrics) / 100),  # Higher confidence with more data
            time_horizon=time_horizon,
    ...
    async def optimize_allocation_strategy(self, current_usage: ResourceMetrics, tasks: List[Task] = None) -> AllocationStrategy:
        ...
        elif all(hasattr(task, 'priority') and getattr(task, 'priority', 5) <= 3 for task in tasks):
            return AllocationStrategy.COST_OPTIMIZED
        else:
            return AllocationStrategy.BALANCED
    
    async def implement_cost_optimization(self, allocation_plan: AllocationPlan) -> AllocationPlan:
        """Optimize allocation plan for cost efficiency."""
        # Calculate current cost
        current_cost = (
            allocation_plan.cpu_allocation * self.cost_per_unit[ResourceType.CPU] * (allocation_plan.estimated_duration / 60) +
            allocation_plan.memory_allocation / 1024 * self.cost_per_unit[ResourceType.MEMORY] * (allocation_plan.estimated_duration / 60) +
            allocation_plan.disk_allocation * self.cost_per_unit[ResourceType.DISK] * (allocation_plan.estimated_duration / 60)
        )
        ...
        optimized_plan = AllocationPlan(
            ...
            estimated_duration=int(allocation_plan.estimated_duration * 1.05),  # Slight time increase
    ...
    async def scale_resources_dynamically(self, demand: ResourceDemand) -> List[ScalingRecommendation]:
        ...
        if demand.cpu_demand > 85:
            recommendations.append(ScalingRecommendation(
                ...
                estimated_cost=self.cost_per_unit[ResourceType.CPU] * 0.2 * 24,  # Cost for 20% increase for 24h
            ...
            recommendations.append(ScalingRecommendation(
                ...
                urgency="low",
                estimated_cost=-self.cost_per_unit[ResourceType.CPU] * 0.15 * 24,  # Negative cost (savings)
    ...
    async def allocate_resources_for_task(self, task: Task, strategy: AllocationStrategy = None) -> AllocationPlan:
        ...
        priority_multiplier = 1.0 + (5 - priority) * 0.1  # Higher priority gets more resources
        ...
        cost_estimate = (
            ...
            base_memory / 1024 * self.cost_per_unit[ResourceType.MEMORY] * (estimated_duration / 60) +
            base_disk * self.cost_per_unit[ResourceType.DISK] * (estimated_duration / 60)
        )
    ...
    def _analyze_task_complexity(self, task: Task) -> float:
        ...
        if hasattr(task, 'config') and 'description' in task.config:
            ...
            complexity_indicators = [
                'analyze', 'optimize', 'process', 'transform', 'compute',
                'machine learning', 'deep learning', 'big data', 'parallel'
            ]
    ...
    async def get_resource_utilization_report(self) -> Dict:
        ...
        total_cpu_allocated = sum(plan.cpu_allocation for plan in self.current_allocations.values())
        total_memory_allocated = sum(plan.memory_allocation for plan in self.current_allocations.values())
        total_cost = sum(plan.cost_estimate for plan in self.current_allocations.values())
        ...
        return {
            ...
            "efficiency_metrics": {
                "cpu_efficiency": min(100, (total_cpu_allocated / max(1, current_metrics.cpu_percent / 100)) * 100),
                "memory_efficiency": min(100, (total_memory_allocated / 16384) * 100),  # Assuming 16GB total
                "cost_efficiency": "optimal" if total_cost < 10.0 else "review_recommended"
            }
        }
