...
class CronValidator:
    ...
    @classmethod
    def get_next_run_time(cls, expression: str, from_time: datetime = None, 
...
class WorkflowScheduler:
    ...
    async def create_schedule(self, workflow_id: str, name: str, cron_expression: str,
                            timezone_str: str = "UTC", created_by: str = None) -> str:
        ...
        with SessionLocal() as db:
            ...
            if not workflow.is_active:
                raise SchedulingError(f"Cannot schedule inactive workflow: {workflow_id}")
            
            # Calculate next run time
            next_run_at = CronValidator.get_next_run_time(cron_expression, None, timezone_str)
    ...
    async def update_schedule(self, schedule_id: str, **updates) -> bool:
        ...
        with SessionLocal() as db:
            ...
            if 'cron_expression' in updates:
                is_valid, error_msg = CronValidator.validate_expression(updates['cron_expression'])
                if not is_valid:
                    raise SchedulingError(f"Invalid cron expression: {error_msg}")
    ...
    async def list_schedules(self, workflow_id: str = None, status: ScheduleStatus = None) -> List[Dict]:
    ...
    async def _check_and_execute_schedules(self):
        ...
        with SessionLocal() as db:
            ...
            due_schedules = db.query(WorkflowSchedule).filter(
                and_(
            ...
            for schedule in due_schedules:
                try:
                    ...
                    logger.error(f"Failed to execute scheduled workflow {schedule.id}: {e}")
    
    async def _execute_scheduled_workflow(self, schedule: WorkflowSchedule, db: Session):
        """Execute a scheduled workflow"""
        
        logger.info(f"Executing scheduled workflow: {schedule.workflow_id} (schedule: {schedule.id})")
        ...
        try:
            ...
            logger.info(f"Scheduled workflow executed: {execution_id}, next run: {schedule.next_run_at}")
            
        except Exception as e:
            logger.error(f"Failed to execute scheduled workflow {schedule.workflow_id}: {e}")
