...
class TaskBatcher:
    ...
    def get_batchable_tasks(self, db: Session) -> List[List[Task]]:
        ...
        batches.sort(key=lambda batch: max(task.priority for task in batch), reverse=True)
        
        return batches
    
    def _group_tasks_by_type_and_priority(self, tasks: List[Task]) -> Dict[str, List[Task]]:
        groups = {}
    ...
    def _create_batches_from_group(self, db: Session, tasks: List[Task]) -> List[List[Task]]:
        if not tasks:
            return []
        ...
        logger.info(f"Created {len(batches)} batches from {len(deduplicated_tasks)} tasks")
        return batches
    ...
    def _is_recent_duplicate(self, db: Session, task: Task, task_hash: str) -> bool:
        ...
        recent_tasks = TaskCRUD.get_similar_tasks(db, task.type, self.deduplication_window_hours)
