...
class QuotaManager:
        ...
        for i, api_key in enumerate(api_keys):
            ...
            if not existing_agent:
                ...
                agent_data = AgentCreate(
                    ...
                    quota_reset_at=datetime.utcnow() + timedelta(hours=self.reset_hours)
                )
                AgentCRUD.create_agent(db, agent_data)
                logger.info(f"Created agent {agent_id} with quota limit {self.quota_limit}")
            else:
    ...
    def consume_quota(self, db: Session, agent_id: str, messages_used: int = 1) -> bool:
        ...
        if agent.quota_used + messages_used > agent.quota_limit:
            logger.warning(f"Agent {agent_id} quota exceeded: {agent.quota_used}/{agent.quota_limit}")
            return False
        ...
        logger.info(f"Consumed {messages_used} quota for agent {agent_id}: {agent.quota_used + messages_used}/{agent.quota_limit}")
        return True
    
    def _needs_quota_reset(self, agent: Agent) -> bool:
        return datetime.utcnow() >= agent.quota_reset_at
