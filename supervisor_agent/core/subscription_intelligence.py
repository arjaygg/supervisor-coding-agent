...
class RequestDeduplicator:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_size_mb": sum(
                len(str(entry.result)) for entry in self.cache.values()
...
class BatchProcessor:
    ...
    async def _timeout_handler(self):
        ...
        try:
            ...
            async with self._batch_lock:
                if self.current_batch:
                    logger.debug(f"Processing batch due to timeout: {len(self.current_batch)} requests")
                    await self._process_current_batch()
    ...
    def get_batch_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batch_age_seconds": (
                ...
                if self.batch_start_time else 0
            ),
...
class UsagePredictor:
    ...
    def record_usage(
        ...
        logger.debug(f"Recorded usage: {task_type}, {tokens_used} tokens, {processing_time:.2f}s")
    
    def predict_tokens(self, task_type: str) -> int:
        """Predict token usage for a task type."""
        # Filter recent history for this task type
        recent_records = [
            record for record in self.usage_history[-100:]  # Last 100 records
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_tokens_per_request": (
                sum(r.tokens_used for r in successful_records) // max(len(successful_records), 1)
            ),
...
class SubscriptionIntelligence:
    ...
    async def process_request(
        ...
        if self.should_batch_request(request):
            ...
            return await self._process_individual_request(request, processor, estimated_tokens)
        ...
        try:
            ...
            for request in batch_requests:
                ...
                if processor and request_id:
                    processor_groups[id(processor)].append((request, request_id, processor))
            ...
            for group in processor_groups.values():
                ...
                try:
                    ...
                    # Handle results
                    for i, (request, request_id) in enumerate(zip(requests, request_ids)):
                        result = results[i] if isinstance(results, list) and i < len(results) else results
