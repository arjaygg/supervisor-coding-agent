...
class NotificationManager:
    ...
    async def send_task_failure_alert(self, task_id: int, task_type: str, error_message: str, retry_count: int):
        message = self._format_task_failure_message(task_id, task_type, error_message, retry_count)
        await self._send_notification(
    ...
    async def send_agent_recovery_notification(self, agent_id: str, recovery_time: datetime):
        message = f"""
...
The agent is now ready to process new tasks.
        ...
        await self._send_notification(
            subject=f"🔄 Agent {agent_id} Recovered",
            message=message,
            priority="low"
        )
...
**Action Required:** Consider increasing quota limits or adding more agents if this becomes frequent.
    ...
    def _format_task_failure_message(self, task_id: int, task_type: str, error_message: str, retry_count: int) -> str:
        return f"""
❌ **Task Failure Alert**
...
**Action Required:** Please check system components and resolve any connectivity issues.
    ...
    async def _send_notification(self, subject: str, message: str, priority: str = "medium"):
        tasks = []
    ...
    def test_notifications(self) -> Dict[str, bool]:
        ...
        if self.email_enabled:
            try:
                import asyncio
                asyncio.run(self._send_email_notification("Test Notification", test_message, "low"))
                results["email"] = True
            except Exception as e:
