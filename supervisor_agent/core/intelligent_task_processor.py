...
class IntelligentTaskProcessor:
    ...
    async def process_task(
        ...
        try:
            ...
            result["optimization_metadata"] = {
                ...
                "tokens_estimated": self.subscription_intelligence.estimate_token_usage(request),
                "batch_processed": len(request) > 1 if isinstance(request, list) else False
            }
            ...
            logger.error(f"Error in intelligent task processing for task {task.id}: {e}")
    ...
    async def process_batch(
        ...
        try:
            ...
            if batchable_tasks:
                batch_requests = [self._task_to_request(task) for task in batchable_tasks]
    ...
    def should_batch_task(self, task: Task) -> bool:
        ...
        try:
            ...
            logger.warning(f"Error determining batch eligibility for task {task.id}: {e}")
            return False
