...
from supervisor_agent.core.provider_coordinator import ProviderCoordinator, ExecutionContext, TaskAffinityStrategy
...
class MultiProviderTaskProcessor:
    ...
    def __init__(
        ...
        logger.info(f"Multi-Provider Task Processor initialized with strategy: {default_routing_strategy}")
    ...
    async def process_task(
        ...
        try:
            ...
            selected_provider_id = await self.provider_coordinator.select_provider(task, context)
            ...
            await self._update_processing_stats(selected_provider_id, result, execution_time)
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        for provider_id in self.provider_registry.providers.keys():
            provider_stats[provider_id] = {
                ...
                "quota_status": await self.subscription_intelligence.get_quota_status(provider_id)
            }
        ...
        stats["system_status"] = {
            "active_providers": len([p for p in self.provider_registry.providers.keys() 
    ...
    async def _check_task_cache(self, task: Task, shared_memory: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            optimal_provider = await self.subscription_intelligence.get_optimal_provider(request_data)
    ...
    async def _execute_task_with_provider(
        ...
        try:
            ...
            if not response.success:
                raise ProviderError(f"Provider execution failed: {response.error_message}")
            ...
            await self.subscription_intelligence.track_request(
                ...
                cost_usd=float(response.cost_estimate.total_cost_usd) if response.cost_estimate else 0.0,
                success=True
            )
            
            # Update task affinity tracking
            self.provider_coordinator.task_affinity_tracker.record_assignment(task, provider_id)
            ...
            logger.debug(f"Task {task.id} executed successfully with provider {provider_id}")
            return result
    ...
    async def _handle_task_failure(
        ...
        if retry_count < self.max_retry_attempts:
            logger.info(f"Retrying task {task.id} (attempt {retry_count + 1}/{self.max_retry_attempts})")
            
            # Get backup provider
            try:
                failed_provider_id = context.exclude_providers[-1] if context.exclude_providers else None
                backup_provider_id = await self.provider_coordinator.select_backup_provider(
                ...
                if backup_provider_id:
                    ...
                    return await self._execute_task_with_provider(
                        task, backup_provider_id, agent_processor, context, shared_memory
                    )
    ...
    async def _optimize_task_batch(
        ...
        try:
            ...
            logger.debug(f"Optimized {len(task_batch.tasks)} tasks into {len(optimized_batches)} batches")
    ...
    async def _is_provider_available(self, provider_id: str) -> bool:
        ...
        try:
            quota_status = await self.subscription_intelligence.get_quota_status(provider_id)
            return bool(quota_status and quota_status[provider_id] and quota_status[provider_id].is_available)
        except Exception:
            return False