...
class EnhancedAgentManager:
    ...
    def __init__(self, enable_multi_provider: bool = None):
        ...
        self.enable_multi_provider = (
            enable_multi_provider if enable_multi_provider is not None 
        ...
        logger.info(f"Enhanced Agent Manager initialized (multi-provider: {self.enable_multi_provider})")
    ...
    async def execute_task(
        ...
        try:
            ...
            if self.enable_multi_provider and prefer_provider:
                return await self._execute_with_multi_provider(task, context, shared_memory)
            elif agent_id:
                return await self._execute_with_legacy_agent(task, agent_id, shared_memory)
            else:
                return await self._execute_with_auto_selection(task, shared_memory, context)
        ...
        try:
            ...
            for agent_id in legacy_agent_ids:
                ...
                migration_plan["legacy_agents"].append({
                    "id": agent_id,
                    "api_key": agent.api_key if hasattr(agent, 'api_key') else "***",
                    "status": "active"
                })
                ...
                provider_mapping = {
                    ...
                    "config": {
                        "api_keys": [agent.api_key] if hasattr(agent, 'api_key') else [],
                        "rate_limit_per_day": 1000,
                        "priority": int(agent_id.split('-')[-1]) if '-' in agent_id else 1
                    }
                }
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        if self.enable_multi_provider:
            try:
                ...
                health["multi_provider_system"] = {
                    "status": "healthy" if status.get("healthy_providers", 0) > 0 else "unhealthy",
        ...
        if health["legacy_system"]["agents"] == 0 and health["multi_provider_system"]["providers"] == 0:
            health["overall_status"] = "critical"
            health["recommendations"].append("No execution methods available - configure agents or providers")
            
        elif health["legacy_system"]["agents"] > 0 and not self.enable_multi_provider:
            health["recommendations"].append("Consider enabling multi-provider system for improved reliability")
            
        elif health["multi_provider_system"]["providers"] == 0 and self.enable_multi_provider:
            health["recommendations"].append("Multi-provider enabled but no providers configured")
