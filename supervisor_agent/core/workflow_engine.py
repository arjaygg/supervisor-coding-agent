...
from supervisor_agent.core.workflow_models import (ExecutionPlan, Workflow,
                                                   WorkflowContext,
                                                   WorkflowDefinition,
                                                   WorkflowExecution,
                                                   WorkflowResult,
...
class WorkflowEngine:
    ...
    async def create_workflow(
        self, workflow_def: WorkflowDefinition, created_by: str = None
    ) -> str:
        ...
        if not validation_result.is_valid:
            raise ValueError(
                f"Invalid workflow definition: {validation_result.error_message}"
            )
    ...
    async def execute_workflow(
        self, workflow_id: str, context: Dict[str, Any] = None, triggered_by: str = None
    ) -> str:
        ...
        with SessionLocal() as db:
            ...
            executor = WorkflowExecutor(
                ...
                context=WorkflowContext(
                    workflow_execution_id=execution_id, variables=context or {}
                ),
        ...
        with SessionLocal() as db:
            ...
            for task_exec in task_executions:
                task_status[task_exec.task_name] = {
                    ...
                    "started_at": (
                        task_exec.started_at.isoformat()
                        if task_exec.started_at
                        else None
                    ),
                    "completed_at": (
                        task_exec.completed_at.isoformat()
            ...
            return {
                ...
                "completed_at": (
                    db_execution.completed_at.isoformat()
                    if db_execution.completed_at
                    else None
                ),
    ...
    async def cancel_execution(self, execution_id: str) -> bool:
        ...
        with SessionLocal() as db:
            ...
            if db_execution and db_execution.status in [
    ...
    async def _run_workflow_execution(self, executor: "WorkflowExecutor"):
        ...
        try:
            ...
            with SessionLocal() as db:
                db_execution = (
                    db.query(WorkflowExecution)
                    .filter(WorkflowExecution.id == executor.execution_id)
                    .first()
                )
...
class WorkflowExecutor:
    ...
    async def _execute_task(self, task_def) -> Dict[str, Any]:
        ...
        try:
            ...
            error_result = {"status": "FAILED", "success": False, "error": str(e)}
    ...
    async def _wait_for_task_completion(
        self, task_id: int, task_name: str, timeout: int = 300
    ) -> Dict[str, Any]:
        """Wait for task completion with polling"""
    ...
    async def _update_task_execution_result(
