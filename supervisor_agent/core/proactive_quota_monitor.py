...
class ProactiveQuotaMonitor:
    ...
    async def check_quota_status(self, provider_id: str) -> SubscriptionStatus:
        ...
        try:
            ...
            usage_percentage = (agent.quota_used / agent.quota_limit * 100) if agent.quota_limit > 0 else 0
            
            # Determine alert level
            alert_level = self._calculate_alert_level(usage_percentage)
            
            # Get usage pattern from predictor
            usage_pattern = await self.usage_predictor.analyze_usage_patterns(provider_id)
    ...
    async def should_defer_task(self, task: Task, provider_id: str) -> Tuple[bool, Optional[str]]:
            ...
            try:
                ...
                if prediction and prediction.severity in ["critical", "emergency"]:
                    ...
                    if status.quota_used + estimated_tokens > status.quota_limit:
                        return True, f"Task would exceed quota limit (estimated {estimated_tokens} tokens)"
                    
                    # Check if we're in high-risk zone
                    if status.current_alert_level in [AlertLevel.CRITICAL, AlertLevel.EMERGENCY]:
                        # Only allow high priority tasks
                        priority = getattr(task, 'priority', 5)  # Default medium priority
                        if priority > 2:  # Only high priority (1-2) allowed
                            return True, f"Deferring non-critical task due to {status.current_alert_level} alert"
    ...
    async def defer_task(
        ...
        try:
            ...
            update_data = crud.schemas.TaskUpdate(
                ...
                error_message=f"Deferred: {reason}"
            )
    ...
    async def resume_deferred_tasks(self, provider_id: str) -> int:
        ...
        status = await self.check_quota_status(provider_id)
        if not status.is_available:
            logger.debug(f"Provider {provider_id} still unavailable, cannot resume tasks")
            return 0
        ...
        for group in self.deferred_tasks[provider_id]:
            for task_id in group.tasks:
                all_deferred.append((task_id, group.priority_level, group.created_at, group.retry_after))
            ...
            try:
                ...
                if task:
                    ...
                    if estimated_tokens <= available_quota:
                        ...
                        logger.info(f"Resumed deferred task {task_id} for provider {provider_id}")
                    else:
        ...
        self._cleanup_deferred_tasks(provider_id, [tid for tid, _, _, _ in all_deferred[:resumed_count]])
        
        if resumed_count > 0:
            logger.info(f"Resumed {resumed_count} deferred tasks for provider {provider_id}")
    ...
    async def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        for provider_id, sub_status in self.subscription_status.items():
            status["subscriptions"][provider_id] = {
                ...
                "usage_percentage": (sub_status.quota_used / sub_status.quota_limit * 100) if sub_status.quota_limit > 0 else 0,
    ...
    async def _check_alert_conditions(
        ...
        usage_percentage = (status.quota_used / status.quota_limit * 100) if status.quota_limit > 0 else 0
        ...
        if status.current_alert_level == AlertLevel.EMERGENCY:
            should_alert = True
            alert_message = f"EMERGENCY: Provider {provider_id} quota at {usage_percentage:.1f}%"
        elif status.current_alert_level == AlertLevel.CRITICAL:
            should_alert = True
            alert_message = f"CRITICAL: Provider {provider_id} quota at {usage_percentage:.1f}%"
            if prediction:
            ...
            should_alert = True
            alert_message = f"WARNING: Provider {provider_id} quota at {usage_percentage:.1f}%"
        ...
        if should_alert:
            alert = QuotaAlert(
                ...
                predicted_exhaustion=prediction.predicted_exhaustion_time if prediction else None,
    ...
    async def _send_alert(self, alert: QuotaAlert) -> None:
        ...
        if len(self.active_alerts[alert.provider_id]) > max_alerts:
            self.active_alerts[alert.provider_id] = self.active_alerts[alert.provider_id][-max_alerts:]
    ...
    def _cleanup_deferred_tasks(self, provider_id: str, completed_task_ids: List[int]) -> None:
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        for provider_id in list(self.active_alerts.keys()):
            self.active_alerts[provider_id] = [
                alert for alert in self.active_alerts[provider_id]
        ...
        for provider_id in list(self.deferred_tasks.keys()):
            self.deferred_tasks[provider_id] = [
                group for group in self.deferred_tasks[provider_id]
