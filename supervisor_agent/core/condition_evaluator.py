...
class SafeEvaluator:
    ...
    ALLOWED_NODES = {
        ...
        ast.Is, ast.IsNot, ast.In, ast.NotIn, ast.BinOp, ast.Add, ast.Sub,
        ...
        ast.SetComp, ast.comprehension, ast.If, ast.IfExp
    }
    
    # Allowed built-in functions
    ALLOWED_BUILTINS = {
        'len', 'str', 'int', 'float', 'bool', 'list', 'tuple', 'dict', 'set',
        'min', 'max', 'sum', 'any', 'all', 'abs', 'round', 'sorted', 'reversed',
        'enumerate', 'zip', 'range'
    }
    ...
    def _is_safe_attribute_access(self, attr_node: ast.Attribute) -> bool:
        ...
        safe_attributes = {
            ...
            'get', 'keys', 'values', 'items', 'append', 'extend',
            'count', 'index', 'find', 'startswith', 'endswith'
        }
...
class ConditionEvaluator:
    ...
    def evaluate_condition(
        ...
        try:
            ...
            logger.debug(f"Condition evaluated in {execution_time:.3f}s: {condition} -> {result}")
            
            return bool(result)
            
        except Exception as e:
            execution_time = (datetime.now() - start_time).total_seconds()
            logger.error(f"Condition evaluation failed after {execution_time:.3f}s: {condition}, error: {str(e)}")
            raise
    ...
    def _evaluate_simple_comparison(self, condition: str, context: Dict[str, Any]) -> bool:
    ...
    def _evaluate_custom_function(self, condition: str, context: Dict[str, Any]) -> bool:
    ...
    def _between(self, context: Dict[str, Any], value_key: str, min_val: float, max_val: float) -> bool:
    ...
    def _matches_pattern(self, context: Dict[str, Any], value_key: str, pattern: str) -> bool:
    ...
    def _count_greater_than(self, context: Dict[str, Any], collection_key: str, threshold: int) -> bool:
