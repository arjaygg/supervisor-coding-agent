...
class MultiProviderSubscriptionIntelligence:
        ...
        try:
            ...
            if success and response:
                ...
                self._cache[request_hash] = CacheEntry(
                    result=response, timestamp=time.time(), hit_count=0
                )
            ...
            await self._persist_usage_data(
                provider_id, request_data, cost_usd, execution_time, success
            )

            logger.debug(
                f"Tracked request for provider {provider_id}: cost=${cost_usd:.4f}, time={execution_time:.2f}s, success={success}"
            )
    ...
    ) -> Dict[str, ProviderQuotaInfo]:
        """Get quota status for one or all providers"""
        await self._update_quota_information()

        if provider_id:
            return (
                {provider_id: self.provider_quotas.get(provider_id)}
                if provider_id in self.provider_quotas
                else {}
            )
        ...
        try:
            ...
            for times in self._response_times.values():
                all_response_times.extend(
                    times[-100:]
                )  # Last 100 requests per provider
            avg_response_time = (
            ...
            most_used = (
                max(self._daily_usage[today].items(), key=lambda x: x[1])[0]
                if self._daily_usage[today]
                else "none"
            )
            ...
            for provider_id, costs in self._cost_tracking.items():
                today_costs = [
                    cost for _, cost in costs if _.date().isoformat() == today
                ]
                ...
                if today_costs and today_requests > 0:
                    ...
                    cost_effectiveness[provider_id] = (
                        1.0 / cost_per_request if cost_per_request > 0 else 0.0
                    )

            most_cost_effective = (
                max(cost_effectiveness.items(), key=lambda x: x[1])[0]
    ...
    async def predict_quota_exhaustion(self, provider_id: str) -> Optional[datetime]:
        ...
        exhaustion_time = datetime.now(timezone.utc) + timedelta(
    ...
    async def _update_provider_quota(
        self, provider_id: str, provider: AIProvider
    ) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Could not update quota for provider {provider_id}: {str(e)}"
            )
    ...
    def _update_quota_status(self, provider_id: str) -> None:
        ...
        if (
            quota_info.status in [QuotaStatus.WARNING, QuotaStatus.CRITICAL]
            and provider_id not in self._quota_warnings_sent
        ):
            logger.warning(
                f"Provider {provider_id} quota at {usage_pct:.1f}% - status: {quota_info.status}"
            )
            self._quota_warnings_sent.add(provider_id)

    async def _get_available_providers(
        self, exclude_providers: Optional[List[str]] = None
    ) -> List[str]:
    ...
    async def _calculate_provider_score(
        ...
        try:
            ...
            quota_score = 1.0 - (
                quota_info.usage_percentage / 100.0
            )  # More available quota = higher score
            cost_score = 1.0 - min(
                1.0, float(cost_estimate.total_cost_usd) / 1.0
            )  # Lower cost = higher score
            performance_score = (
                quota_info.success_rate
            ...
            logger.error(
                f"Error calculating provider score for {provider_id}: {str(e)}"
            )
            return 0.0
    ...
    async def _persist_usage_data(
        ...
        try:
            ...
            logger.debug(
                f"Persisting usage data: provider={provider_id}, cost={cost_usd}, time={execution_time}, success={success}"
            )
        ...
        try:
            ...
            for provider_id, quota_info in self.provider_quotas.items():
                if quota_info.usage_percentage < 20:
                    recommendations.append(
                        f"Provider {provider_id} is underutilized ({quota_info.usage_percentage:.1f}% used)"
                    )
            ...
            total_cost_today = sum(
                sum(
                    cost
                    for _, cost in costs
                    if _.date() == datetime.now(timezone.utc).date()
                )
                for costs in self._cost_tracking.values()
            )
            ...
            if total_cost_today > 10.0:  # Arbitrary threshold
                recommendations.append(
                    f"Daily cost is high (${total_cost_today:.2f}) - consider optimizing provider selection"
                )
            ...
            for provider_id, times in self._response_times.items():
                if times and len(times) >= 5:
                    ...
                    if avg_time > 15.0:  # 15 seconds threshold
                        recommendations.append(
                            f"Provider {provider_id} has slow response times ({avg_time:.1f}s average)"
                        )
