...
class AdvancedUsagePredictor:
    ...
    async def record_usage(
        ...
        if (
            self.pattern_update_time is None
            or timestamp - self.pattern_update_time > timedelta(hours=1)
        ):
    ...
    async def predict_usage(
        ...
        if len(self.usage_history) < self.min_samples_for_ml:
            ...
            result = await self._simple_prediction(
                provider_id, horizon_minutes, task_type
            )
        else:
            # Use ML models
            result = await self._ml_prediction(
                features, horizon_minutes, provider_id, task_type
            )
    ...
    async def predict_quota_exhaustion(
        ...
        for hours, pred in predictions:
            ...
            if cumulative_usage >= quota_limit:
                ...
                prev_hours = (
                    predictions[predictions.index((hours, pred)) - 1][0]
                    if predictions.index((hours, pred)) > 0
                    else 0
                )
                prev_usage = current_usage + (
                    predictions[predictions.index((hours, pred)) - 1][1].predicted_value
                    if predictions.index((hours, pred)) > 0
                    else 0
                )
                ...
                usage_rate = (
                    (cumulative_usage - prev_usage) / (hours - prev_hours)
                    if hours > prev_hours
                    else 0
                )
                if usage_rate > 0:
                    remaining_in_period = quota_limit - prev_usage
                    hours_to_exhaustion = prev_hours + (
                        remaining_in_period / usage_rate
                    )
        ...
        # Determine severity
        hours_to_exhaustion = (
            exhaustion_time - datetime.now(timezone.utc)
        ).total_seconds() / 3600
    ...
    async def analyze_usage_patterns(
        ...
        if self.current_pattern and self.pattern_update_time:
            if datetime.now(timezone.utc) - self.pattern_update_time < timedelta(
                hours=1
            ):
        ...
        usage_data = [
            ...
            or getattr(record, "provider_id", None) == provider_id
        ]
        ...
        if len(hourly_usage) > 1:
            volatility = statistics.stdev(hourly_usage) / max(
                statistics.mean(hourly_usage), 1
            )
        else:
            volatility = 0.0
    ...
    async def get_optimization_recommendations(self, provider_id: str) -> List[str]:
        ...
        try:
            ...
            recommendations.append(
                "Unable to generate recommendations due to insufficient data"
            )
    ...
    async def _simple_prediction(
        ...
        recent_usage = [
            ...
            if (
                ...
                and getattr(record, "provider_id", None) == provider_id
            )
        ]
    ...
    async def _ml_prediction(
        ...
        try:
            ...
            pred_std = (
                statistics.stdev(predictions.values()) if len(predictions) > 1 else 0
            )
    ...
    async def _evaluate_models(self, X: np.ndarray, y: List[float]):
        ...
        for model_type, model in self.models.items():
            try:
                ...
                logger.debug(
                    f"Model {model_type}: MAE={mae:.2f}, Performance={performance:.3f}"
                )
    ...
    def _aggregate_usage_by_day(self, usage_data: List[UsageRecord]) -> List[float]:
        ...
        for record in usage_data:
            day_key = record.timestamp.replace(
    ...
    def _find_peak_hours(self, hourly_usage: List[float]) -> List[int]:
        ...
        hourly_avgs = [hourly_sums[i] / max(hourly_counts[i], 1) for i in range(24)]
        ...
        if len(hourly_avgs) > 0:
            ...
            peak_hours = [
                hour
                for hour, avg in enumerate(hourly_avgs)
    ...
    def _calculate_usage_distribution(
        ...
        small_tasks = sum(
            record.tokens_used for record in usage_data if record.tokens_used < 1000
        )
        medium_tasks = sum(
            record.tokens_used
            for record in usage_data
        ...
        large_tasks = sum(
            record.tokens_used for record in usage_data if record.tokens_used >= 5000
        )
