...
def analyze_repository_task(self, repo_path: str, task_id: Optional[int] = None):
    ...
    try:
        ...
        if task_id:
            ...
            asyncio.run(notify_task_update({
                "id": task_id,
        ...
        analysis_data = {
            ...
            "unified_insights": {
                ...
                "technical_debt_count": len(result.unified_insights.technical_debt_indicators),
                "priority_recommendations_count": len(result.unified_insights.priority_recommendations)
            },
            "scc_summary": {
                ...
                "primary_languages": result.scc_result.repository_summary["primary_languages"][:3]
            },
            "semgrep_summary": {
                "total_findings": result.semgrep_result.summary["total_findings"],
                "critical_issues": result.semgrep_result.summary["by_severity"]["ERROR"],
                "warning_issues": result.semgrep_result.summary["by_severity"]["WARNING"],
        ...
        if self.request.retries < self.max_retries:
            logger.info(f"Retrying static analysis for {repo_path} (attempt {self.request.retries + 1})")
...
def analyze_files_task(self, file_paths: List[str], base_path: Optional[str] = None, task_id: Optional[int] = None):
    """
...
def batch_repository_analysis(repo_paths: List[str]):
    ...
    try:
        ...
        logger.info(f"Batch analysis completed: {success_count}/{len(repo_paths)} successful")
    ...
    try:
        ...
        if result["risk_profile"] in ["HIGH", "CRITICAL"]:
            alerts.append({
                ...
                "severity": "ERROR" if result["risk_profile"] == "CRITICAL" else "WARNING"
            })
        ...
        if alerts:
            logger.warning(f"Repository health alerts for {repo_path}: {len(alerts)} issues")
