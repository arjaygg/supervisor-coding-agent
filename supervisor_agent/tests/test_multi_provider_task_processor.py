...
from supervisor_agent.core.multi_provider_task_processor import (
    MultiProviderTaskProcessor, RoutingStrategy, TaskPriority, TaskBatch
)
...
from supervisor_agent.providers.base_provider import ProviderResponse, CostEstimate
...
def mock_provider():
    ...
    provider = Mock()
    provider.execute_task = AsyncMock(return_value=ProviderResponse(
        success=True,
...
@pytest.fixture
def task_processor(mock_provider_registry, mock_provider_coordinator, mock_subscription_intelligence):
...
class TestMultiProviderTaskProcessor:
    ...
    @pytest.mark.asyncio
    async def test_process_single_task_success(self, task_processor, sample_tasks, mock_provider):
    ...
    @pytest.mark.asyncio
    async def test_process_task_with_context(self, task_processor, sample_tasks, mock_provider):
    ...
    @pytest.mark.asyncio
    async def test_process_task_with_routing_strategy(self, task_processor, sample_tasks, mock_provider):
    ...
    async def test_process_task_cache_hit(self, task_processor, sample_tasks):
        ...
        # Mock cache hit
        with patch.object(task_processor, '_check_task_cache', return_value={
            "success": True,
    ...
    @pytest.mark.asyncio
    async def test_process_task_failure_with_retry(self, task_processor, sample_tasks, mock_provider):
        ...
        failing_provider.execute_task = AsyncMock(side_effect=[
        ...
        task_processor.provider_registry.get_provider = Mock(return_value=failing_provider)
    ...
    @pytest.mark.asyncio
    async def test_process_task_batch(self, task_processor, sample_tasks, mock_provider):
    ...
    async def test_provider_availability_check(self, task_processor):
        ...
        task_processor.subscription_intelligence.get_quota_status = AsyncMock(return_value={
            "provider-1": Mock(is_available=True)
        })
        ...
        task_processor.subscription_intelligence.get_quota_status = AsyncMock(return_value={
            "provider-2": Mock(is_available=False)
        })
...
@pytest.mark.asyncio
async def test_full_processing_workflow(mock_provider_registry, mock_provider_coordinator, 
                                      mock_subscription_intelligence, sample_tasks, mock_provider):
