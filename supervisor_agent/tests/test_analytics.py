...
class TestAnalyticsEngine:
    ...
    @pytest.mark.asyncio
    async def test_process_metrics_with_cache_miss(self, analytics_engine, sample_query):
        ...
        with patch.object(analytics_engine, 'session_factory') as mock_session:
            ...
            with patch.object(analytics_engine, '_execute_query', new_callable=AsyncMock) as mock_execute:
                mock_execute.return_value = []
                
                with patch.object(analytics_engine, '_cache_result', new_callable=AsyncMock):
    ...
    async def test_generate_insights(self, analytics_engine):
        ...
        with patch.object(analytics_engine, 'process_metrics', new_callable=AsyncMock) as mock_process:
    ...
    async def test_real_time_metrics_simulation(self):
        ...
        with patch.object(collector, 'session_factory') as mock_session:
            ...
            for task_id in task_ids:
                ...
                mock_task.status = TaskStatus.COMPLETED if task_id <= 4 else TaskStatus.FAILED
                mock_task.created_at = datetime.now(timezone.utc) - timedelta(minutes=task_id)
