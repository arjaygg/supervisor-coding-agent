...
from supervisor_agent.providers.base_provider import (
    AIProvider, ProviderCapabilities, ProviderHealth, ProviderResponse, 
    CostEstimate, ProviderError
)
...
def mock_provider():
    ...
    provider.estimate_cost = Mock(return_value=CostEstimate(
        base_cost_usd="0.05",
        token_cost_usd="0.02", 
        total_cost_usd="0.07",
...
class TestProviderCoordinator:
    ...
    @pytest.mark.asyncio
    async def test_select_provider_basic(self, mock_registry, sample_task, execution_context):
        """Test basic provider selection"""
        coordinator = ProviderCoordinator(mock_registry)
        
        selected_provider = await coordinator.select_provider(sample_task, execution_context)
    ...
    @pytest.mark.asyncio
    async def test_select_provider_no_suitable_providers(self, mock_registry, sample_task):
        ...
        mock_provider.get_capabilities.return_value = ProviderCapabilities(
            supported_task_types=["DIFFERENT_TYPE"],
            max_concurrent_tasks=10
        )
        ...
        with pytest.raises(ProviderError, match="No providers capable of handling task type"):
            await coordinator.select_provider(sample_task, context)
            
    @pytest.mark.asyncio
    async def test_select_backup_provider(self, mock_registry, sample_task, execution_context):
    ...
    @pytest.mark.asyncio
    async def test_load_balancing_strategies(self, mock_registry, sample_task, execution_context):
    ...
    @pytest.mark.asyncio
    async def test_provider_recommendations(self, mock_registry, sample_task, execution_context):
        """Test provider recommendations functionality"""
        coordinator = ProviderCoordinator(mock_registry)
        
        recommendations = await coordinator.get_provider_recommendations(sample_task, execution_context)
    ...
    async def test_health_caching(self, mock_registry, sample_task, execution_context):
        ...
        call_count_1 = mock_registry.get_provider.return_value.get_health_status.call_count
        
        # Second call within cache TTL should use cached health
        await coordinator.select_provider(sample_task, execution_context)
        call_count_2 = mock_registry.get_provider.return_value.get_health_status.call_count
...
async def test_provider_coordinator_integration(mock_registry, sample_task):
    ...
    recommendations = await coordinator.get_provider_recommendations(sample_task, context)
    assert isinstance(recommendations, list)
