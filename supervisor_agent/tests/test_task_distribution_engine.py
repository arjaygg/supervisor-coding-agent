...
class TestIntelligentTaskSplitter:
    ...
    def test_identify_dependencies(self, task_splitter):
        ...
        content_with_deps = "This requires database access and needs authentication system"
    ...
    def test_determine_complexity_level(self, task_splitter):
        ...
        # Test simple task
        assert task_splitter._determine_complexity_level(0.3, 2) == TaskComplexity.SIMPLE
        
        # Test moderate task
        assert task_splitter._determine_complexity_level(0.8, 4) == TaskComplexity.MODERATE
        
        # Test complex task
        assert task_splitter._determine_complexity_level(1.5, 10) == TaskComplexity.COMPLEX
        
        # Test highly complex task
        assert task_splitter._determine_complexity_level(2.5, 25) == TaskComplexity.HIGHLY_COMPLEX
    ...
    def test_analyze_task_complexity_complex(self, task_splitter, complex_task):
        ...
        assert isinstance(analysis, ComplexityAnalysis)
        assert analysis.complexity_level in [TaskComplexity.MODERATE, TaskComplexity.COMPLEX, TaskComplexity.HIGHLY_COMPLEX]
        assert analysis.estimated_steps > 1
...
class TestDependencyManager:
    ...
    def test_execution_levels_calculation(self, dependency_manager, sample_task_splits):
        ...
        levels = dependency_manager._calculate_execution_levels(sample_task_splits, edges)
...
class TestTaskDistributionEngine:
    ...
    def mock_agent_specialization_engine(self):
        ...
        mock = Mock()
        mock.determine_agent_specialty = AsyncMock(return_value=AgentSpecialty.GENERAL_DEVELOPER)
        return mock
    ...
    def mock_multi_provider_coordinator(self):
        ...
        mock = Mock()
        mock.get_available_providers = AsyncMock(return_value=[ProviderType.CLAUDE_CLI, ProviderType.OPENAI])
        return mock
    
    @pytest.fixture
    def distribution_engine(self, mock_agent_specialization_engine, mock_multi_provider_coordinator):
    ...
    async def test_strategy_optimization(self, distribution_engine, sample_task):
        ...
        assert result_performance.strategy_used == DistributionStrategy.PERFORMANCE_OPTIMIZED
    ...
    def test_execution_cost_estimation(self, distribution_engine):
        ...
        cost = distribution_engine._estimate_execution_cost(task_splits, provider_assignments)
...
class TestTaskSplitGeneration:
    ...
    def test_no_split_strategy(self, distribution_engine, sample_task):
        ...
        splits = distribution_engine._create_task_splits_by_strategy(sample_task, analysis)
...
class TestDistributionStrategies:
    ...
    def test_strategy_optimization_simple_task(self, distribution_engine):
        ...
        optimized = asyncio.run(distribution_engine._optimize_distribution_strategy(
            simple_task,
    ...
    def test_strategy_optimization_with_constraints(self, distribution_engine):
        ...
        cost_optimized = asyncio.run(distribution_engine._optimize_distribution_strategy(
            task,
            DistributionStrategy.DEPENDENCY_AWARE,
            analysis,
            dependency_graph,
        ...
        perf_optimized = asyncio.run(distribution_engine._optimize_distribution_strategy(
            task,
            DistributionStrategy.DEPENDENCY_AWARE,
            analysis,
            dependency_graph,
...
class TestIntegration:
    ...
    async def test_end_to_end_task_distribution(self):
        ...
        assert result.complexity_analysis.complexity_level in [
            TaskComplexity.MODERATE, TaskComplexity.COMPLEX, TaskComplexity.HIGHLY_COMPLEX
        ]
        assert result.execution_plan is not None
    ...
    def test_performance_characteristics(self):
        ...
        for complexity in complexity_levels:
            analysis = ComplexityAnalysis(
                ...
                estimated_steps=complexity.value == 'simple' and 1 or 
                              complexity.value == 'moderate' and 3 or
                              complexity.value == 'complex' and 10 or 25,
                identified_dependencies=[],
                splitting_recommendation=SplittingStrategy.LINEAR_SPLIT,
