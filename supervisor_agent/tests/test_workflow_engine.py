...
class TestWorkflowEngine:
    ...
    async def test_create_workflow_success(self, workflow_engine, simple_workflow_def):
        ...
        with patch('supervisor_agent.core.workflow_engine.SessionLocal') as mock_session:
            mock_db = Mock()
    ...
    @pytest.mark.asyncio
    async def test_create_workflow_invalid_dag(self, workflow_engine, simple_workflow_def):
    ...
    async def test_execute_workflow_success(self, workflow_engine):
        ...
        with patch('supervisor_agent.core.workflow_engine.SessionLocal') as mock_session:
            ...
            with patch('supervisor_agent.core.workflow_engine.WorkflowExecutor') as mock_executor_class:
                mock_executor = Mock()
...
class TestWorkflowExecutor:
    ...
    def mock_dag_resolver(self):
        ...
        execution_plan = ExecutionPlan(
            ...
            task_map={
                "task1": TaskDefinition("task1", "First Task", "CODE_ANALYSIS", {"input": "test"}),
                "task2": TaskDefinition("task2", "Second Task", "PR_REVIEW", {"pr_number": 123})
            },
    ...
    @pytest.fixture
    def workflow_executor(self, mock_dag_resolver, mock_task_processor, simple_workflow_def):
    ...
    def simple_workflow_def(self):
        ...
        return WorkflowDefinition(
            ...
            description="Test",
            tasks=[
                {"id": "task1", "name": "Task 1", "type": "CODE_ANALYSIS", "config": {}},
    ...
    async def test_execute_success(self, workflow_executor):
        ...
        with patch.object(workflow_executor, '_update_execution_status', new_callable=AsyncMock):
            with patch.object(workflow_executor, '_execute_plan', new_callable=AsyncMock):
                with patch.object(workflow_executor, '_has_failed_tasks', return_value=False):
    ...
    async def test_execute_with_failed_tasks(self, workflow_executor):
        ...
        with patch.object(workflow_executor, '_update_execution_status', new_callable=AsyncMock):
            with patch.object(workflow_executor, '_execute_plan', new_callable=AsyncMock):
                with patch.object(workflow_executor, '_has_failed_tasks', return_value=True):
    ...
    async def test_execute_task_success(self, workflow_executor):
        ...
        with patch.object(workflow_executor, '_create_task_record', new_callable=AsyncMock) as mock_create:
            with patch.object(workflow_executor, '_create_task_execution_record', new_callable=AsyncMock):
                with patch.object(workflow_executor, '_wait_for_task_completion', new_callable=AsyncMock) as mock_wait:
                    with patch.object(workflow_executor, '_update_task_execution_result', new_callable=AsyncMock):
                        ...
                        mock_wait.return_value = {"status": "COMPLETED", "success": True, "result": {"output": "test"}}
    ...
    def test_map_task_type(self, workflow_executor):
        ...
        assert workflow_executor._map_task_type("CODE_ANALYSIS") == TaskType.CODE_ANALYSIS
        assert workflow_executor._map_task_type("BUG_FIX") == TaskType.BUG_FIX
        
        # Test unknown type (should default to CODE_ANALYSIS)
        assert workflow_executor._map_task_type("UNKNOWN_TYPE") == TaskType.CODE_ANALYSIS
