...
class TestRealTimeMonitor:
    ...
    async def test_detect_bottlenecks(self, monitor):
        ...
        cpu_bottleneck = next((b for b in bottlenecks if b["type"] == "cpu_bottleneck"), None)
    ...
    async def test_sla_compliance_tracking(self, monitor):
        ...
        for i in range(10):
            monitor._record_metric(MetricType.RESPONSE_TIME, 4000.0)  # Above 3s threshold
    ...
    async def test_performance_dashboard_data(self, monitor):
        ...
        assert health in ["excellent", "good", "fair", "poor", "critical", "unknown"]
...
class TestBottleneckDetector:
    ...
    async def test_analyze_execution_pipeline(self, detector, sample_pipeline):
        ...
        processing_bottleneck = next((b for b in bottleneck_stages if b["stage_name"] == "data_processing"), None)
    ...
    async def test_identify_slow_components(self, detector, sample_component_metrics):
        ...
        slow_components = await detector.identify_slow_components(sample_component_metrics)
        
        assert len(slow_components) > 0
        
        # API gateway should be identified as slow (high response times, CPU usage)
        api_component = next((c for c in slow_components if c["component_name"] == "api_gateway"), None)
        assert api_component is not None
        assert len(api_component["issues"]) > 0
        
        # Check for high response time issue
        response_issue = next((i for i in api_component["issues"] if i["type"] == "slow_response"), None)
        assert response_issue is not None
        
        # Database should be identified for memory issues
        db_component = next((c for c in slow_components if c["component_name"] == "database_pool"), None)
        assert db_component is not None
        
        memory_issue = next((i for i in db_component["issues"] if i["type"] == "high_memory_usage"), None)
    ...
    async def test_suggest_optimization_strategies(self, detector):
        ...
        cpu_recommendations = [r for r in recommendations if "cpu" in r.description.lower()]
        memory_recommendations = [r for r in recommendations if "memory" in r.description.lower()]
        queue_recommendations = [r for r in recommendations if "queue" in r.description.lower()]
    ...
    @pytest.mark.asyncio
    async def test_optimization_report_generation(self, detector, sample_component_metrics):
...
class TestPerformanceOptimizer:
    ...
    @pytest.mark.asyncio
    async def test_analyze_performance_bottlenecks(self, optimizer, sample_metrics_data):
        """Test performance bottleneck analysis."""
        bottlenecks = await optimizer.analyze_performance_bottlenecks(sample_metrics_data)
        
        # Should detect high CPU and response time bottlenecks
        assert len(bottlenecks) > 0
        
        cpu_bottleneck = next((b for b in bottlenecks if b["type"] == "high_resource_usage"), None)
        assert cpu_bottleneck is not None
        assert cpu_bottleneck["severity"] == "high"
        
        response_bottleneck = next((b for b in bottlenecks if b["type"] == "high_response_time"), None)
        assert response_bottleneck is not None

    @pytest.mark.asyncio
    async def test_generate_optimization_recommendations(self, optimizer, sample_metrics_data):
        """Test optimization recommendation generation."""
        analysis = {"current_metrics": sample_metrics_data}
        recommendations = await optimizer.generate_optimization_recommendations(analysis)
        
        assert len(recommendations) > 0
        
        # Should have recommendations for detected bottlenecks
        caching_rec = next((r for r in recommendations if r.technique == OptimizationTechnique.CACHING), None)
        assert caching_rec is not None
        assert caching_rec.target in [OptimizationTarget.RESPONSE_TIME, OptimizationTarget.RESOURCE_UTILIZATION]
    ...
    async def test_analyze_performance_patterns(self, optimizer, sample_time_window):
        ...
        assert business_impact["business_hours_multiplier"] > 1.0  # Should detect higher load during business hours
    ...
    async def test_detect_performance_regressions(self, optimizer):
        ...
        optimizer.performance_metrics["response_time"].append(
            optimizer.PerformanceMetric(name="response_time", value=1500.0, unit="ms")  # 50% worse
        )
        optimizer.performance_metrics["throughput"].append(
            optimizer.PerformanceMetric(name="throughput", value=750.0, unit="rps")  # 25% worse
        )
        optimizer.performance_metrics["error_rate"].append(
            optimizer.PerformanceMetric(name="error_rate", value=2.0, unit="percent")  # 300% worse
        )
        ...
        response_regression = next((r for r in regressions if r["metric"] == "response_time"), None)
        assert response_regression is not None
        assert response_regression["regression_percent"] > 10  # Significant regression
        assert response_regression["severity"] in ["medium", "high", "critical"]
    ...
    def test_correlation_analysis(self, optimizer):
        ...
        for i in range(50):
            ...
            optimizer.performance_metrics["cpu_usage"].append(
                optimizer.PerformanceMetric(name="cpu_usage", value=cpu_value, unit="percent")
            )
            optimizer.performance_metrics["response_time"].append(
                optimizer.PerformanceMetric(name="response_time", value=response_value, unit="ms")
            )
