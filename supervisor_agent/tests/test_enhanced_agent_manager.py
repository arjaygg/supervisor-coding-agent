...
def mock_legacy_manager():
    ...
    manager.get_available_agent_ids = Mock(return_value=["claude-agent-1", "claude-agent-2"])
...
def enhanced_manager(mock_legacy_manager):
    ...
    with patch('supervisor_agent.core.enhanced_agent_manager.AgentManager', return_value=mock_legacy_manager):
...
class TestEnhancedAgentManagerInitialization:
    ...
    def test_initialization_default_settings(self, mock_legacy_manager):
        ...
        with patch('supervisor_agent.core.enhanced_agent_manager.settings') as mock_settings:
...
class TestTaskExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_task_with_multi_provider(self, enhanced_manager_with_providers, sample_task):
        """Test task execution with multi-provider system"""
        with patch('supervisor_agent.core.enhanced_agent_manager.multi_provider_service') as mock_service:
            mock_service.is_enabled.return_value = True
            mock_service.process_task = AsyncMock(return_value={
                "success": True,
    ...
    @pytest.mark.asyncio
    async def test_execute_task_auto_selection(self, enhanced_manager_with_providers, sample_task):
    ...
    @pytest.mark.asyncio
    async def test_execute_task_fallback_on_failure(self, enhanced_manager_with_providers, sample_task):
        """Test fallback to alternative execution method on failure"""
        with patch('supervisor_agent.core.enhanced_agent_manager.multi_provider_service') as mock_service:
            # First attempt fails
            mock_service.is_enabled.return_value = True
            mock_service.process_task = AsyncMock(side_effect=Exception("Provider failure"))
...
class TestAgentAvailability:
    ...
    @pytest.mark.asyncio
    async def test_get_available_agents_with_providers(self, enhanced_manager_with_providers):
...
class TestProviderPreferences:
    ...
    async def test_set_provider_preference(self, enhanced_manager_with_providers):
        ...
        stored_prefs = await enhanced_manager_with_providers.get_provider_preference("user-123")
    ...
    @pytest.mark.asyncio
    async def test_get_nonexistent_provider_preference(self, enhanced_manager_with_providers):
        """Test getting preferences for non-existent user"""
        result = await enhanced_manager_with_providers.get_provider_preference("nonexistent-user")
...
class TestSystemHealth:
    ...
    @pytest.mark.asyncio
    async def test_get_system_health_with_providers(self, enhanced_manager_with_providers):
    ...
    @pytest.mark.asyncio
    async def test_get_system_health_critical_status(self, enhanced_manager_with_providers):
        """Test system health when no execution methods available"""
        # Mock no legacy agents
        enhanced_manager_with_providers.legacy_manager.get_available_agent_ids = Mock(return_value=[])
        ...
        with patch('supervisor_agent.core.enhanced_agent_manager.multi_provider_service') as mock_service:
            ...
            assert any("No execution methods available" in rec for rec in result["recommendations"])
