...
class TestSemgrepAnalyzer:
    ...
    def test_analyze_repository_success(
        self, mock_run, semgrep_analyzer, mock_semgrep_output
    ):
        """Test successful repository analysis."""
        mock_result = Mock()
...
class TestStaticAnalysisPipeline:
    ...
    def test_analyze_repository_sequential(
        ...
        with patch.object(
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                assert (
                    result.pipeline_metrics.parallel_efficiency == 0.0
                )  # No parallelism

    def test_analyze_repository_parallel(
        self, pipeline, mock_scc_result, mock_semgrep_result
    ):
    ...
    def test_unified_insights_generation(
        self, pipeline, mock_scc_result, mock_semgrep_result
    ):
    ...
    def test_technical_debt_identification(
        self, pipeline, mock_scc_result, mock_semgrep_result
    ):
        ...
        debt_indicators = pipeline._identify_technical_debt(
            mock_scc_result, mock_semgrep_result
        )
    ...
    def test_ai_model_context_creation(
        self, pipeline, mock_scc_result, mock_semgrep_result
    ):
