...
class TestPredictiveAnalyticsEngine:
    ...
    @pytest.mark.asyncio
    async def test_predict_workflow_failures(self, analytics_engine, sample_workflow_data):
        """Test workflow failure prediction functionality."""
        prediction = await analytics_engine.predict_workflow_failures(sample_workflow_data)
        ...
        assert overall_risk["level"] in ["very_low", "low", "medium", "high", "critical"]
        assert 0 <= overall_risk["probability"] <= 1
        assert 0 <= overall_risk["confidence"] <= 1
        
        # Test specific risks
        specific_risks = prediction["specific_risks"]
        expected_risk_types = ["resource_exhaustion", "performance_degradation", "dependency_failure", "cascade_failure"]
    ...
    @pytest.mark.asyncio
    async def test_predict_resource_demand(self, analytics_engine, sample_resource_demand_data):
        """Test resource demand forecasting."""
        forecast = await analytics_engine.predict_resource_demand(sample_resource_demand_data)
        ...
        resource_types = ["cpu_demand", "memory_demand", "storage_demand", "network_demand"]
    ...
    @pytest.mark.asyncio
    async def test_analyze_performance_trends(self, analytics_engine, sample_historical_metrics):
        """Test performance trend analysis."""
        trends = await analytics_engine.analyze_performance_trends(sample_historical_metrics)
        ...
        expected_metrics = ["response_time", "throughput", "cpu_usage", "error_rate"]
        ...
        for metric in expected_metrics:
            if metric in trend_results:
                ...
                assert trend_data["direction"] in [
                    ...
                    "decreasing", "strongly_decreasing", "volatile"
                ]
                assert 0 <= trend_data["strength"] <= 1

    @pytest.mark.asyncio
    async def test_seasonal_pattern_detection(self, analytics_engine, sample_historical_metrics):
    ...
    async def test_identify_optimization_opportunities(self, analytics_engine):
        ...
        opportunities = await analytics_engine.identify_optimization_opportunities(system_state)
    ...
    async def test_optimization_opportunity_types(self, analytics_engine):
        ...
        opportunities = await analytics_engine.identify_optimization_opportunities(high_resource_state)
        ...
        cost_opportunities = await analytics_engine.identify_optimization_opportunities(high_cost_state)
    ...
    async def test_time_series_forecasting_models(self, analytics_engine):
        ...
        linear_forecast = analytics_engine._forecast_linear_regression(time_series_data, 3)
        assert len(linear_forecast) == 3
        assert all(isinstance(val, (int, float)) for val in linear_forecast)
        
        # Test exponential smoothing
        exp_forecast = analytics_engine._forecast_exponential_smoothing(time_series_data, 3)
    ...
    @pytest.mark.asyncio
    async def test_risk_assessment_calculation(self, analytics_engine, sample_workflow_data):
        ...
        resource_risk = await analytics_engine._assess_resource_exhaustion_risk(features)
        ...
        perf_risk = await analytics_engine._assess_performance_degradation_risk(features)
    ...
    def test_feature_extraction(self, analytics_engine, sample_workflow_data):
        ...
        expected_features = [
            ...
            "avg_response_time", "throughput", "error_rate", "queue_depth"
        ]
        ...
        derived_features = analytics_engine._calculate_derived_features(features, sample_workflow_data)
    ...
    @pytest.mark.asyncio
    async def test_autocorrelation_analysis(self, analytics_engine, sample_historical_metrics):
        """Test autocorrelation analysis for seasonality detection."""
        # Extract response time values
        response_times = [float(metric["response_time"]) for metric in sample_historical_metrics]
    ...
    @pytest.mark.asyncio
    async def test_model_ensemble_forecasting(self, analytics_engine, sample_resource_demand_data):
        ...
        assert cpu_prediction["model_used"] in [model.value for model in PredictionModel]
    ...
    @pytest.mark.asyncio
    async def test_recommendation_generation(self, analytics_engine, sample_workflow_data):
    ...
    async def test_prediction_caching(self, analytics_engine, sample_workflow_data):
        ...
        # First prediction
        prediction1 = await analytics_engine.predict_workflow_failures(sample_workflow_data)
        ...
        prediction2 = await analytics_engine.predict_workflow_failures(sample_workflow_data)
        
        # Results should be consistent
        assert prediction1["workflow_id"] == prediction2["workflow_id"]
        assert prediction1["overall_risk"]["level"] == prediction2["overall_risk"]["level"]

    @pytest.mark.asyncio
    async def test_confidence_interval_calculation(self, analytics_engine, sample_resource_demand_data):
        ...
        for resource_type in ["cpu_demand", "memory_demand"]:
            if resource_type in confidence_intervals:
                ...
                assert len(intervals["lower_bound"]) == sample_resource_demand_data["forecast_horizon"]
                assert len(intervals["upper_bound"]) == sample_resource_demand_data["forecast_horizon"]
    ...
    async def test_complex_workflow_scenario(self, analytics_engine):
        ...
        complex_workflow = {
            ...
            "task_count": 200,
            "active_users": 500,
            "dependencies": ["service_a", "service_b", "service_c", "database", "cache"],
    ...
    @pytest.mark.asyncio
    async def test_seasonal_decomposition(self, analytics_engine, sample_historical_metrics):
        """Test seasonal decomposition functionality."""
        response_times = [float(metric["response_time"]) for metric in sample_historical_metrics]
        
        decomposition = analytics_engine._seasonal_decomposition(response_times, 24)  # 24-hour period
