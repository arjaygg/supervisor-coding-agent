...
class TestDynamicResourceAllocator:
    ...
    async def test_optimize_allocation_strategy(self, allocator, sample_task):
        ...
        strategy = await allocator.optimize_allocation_strategy(
            high_cpu_metrics, [sample_task]
        )
        ...
        strategy = await allocator.optimize_allocation_strategy(
            high_memory_metrics, [sample_task]
        )
...
class TestResourceConflictResolver:
    ...
    @pytest.mark.asyncio
    async def test_detect_resource_conflicts_comprehensive(
    ...
    async def test_manage_priority_queues(self, resolver):
        ...
        tasks = [
            Task(
                id="urgent_task",
                priority=1,
                config={"description": "urgent task"},
            ),
            Task(
                id="normal_task",
                priority=5,
                config={"description": "normal task"},
            ),
            Task(
                id="low_task",
                priority=8,
                config={"description": "low priority task"},
            ),
            Task(
                id="background_task",
                priority=9,
    ...
    async def test_handle_resource_preemption(self, resolver):
        ...
        assert result["preemption_status"] in [
            "success",
            "no_preemption_needed",
        ]
