...
class TestStrategicPlanner:
    ...
    async def test_strategic_objectives_generation(
        ...
        strategic_planner.claude_agent.execute_task = AsyncMock(
            return_value={
    ...
    async def test_resource_allocation_optimization(
        ...
        if plan.resource_allocation:
            assert (
                "by_resource_type" in plan.resource_allocation
                or "by_objective" in plan.resource_allocation
                or "by_initiative" in plan.resource_allocation
            )
    ...
    async def test_implementation_timeline_creation(
        ...
        if plan.timeline:
            ...
            found_elements = [
                elem for elem in expected_elements if elem in plan.timeline
            ]
    ...
    async def test_strategic_analysis_fallback(
        ...
        strategic_planner.claude_agent.execute_task = AsyncMock(
            return_value={"result": "invalid json response"}
        )
    ...
    async def test_plan_complexity_handling(
        ...
        complex_priorities = [
            f"Priority {i}: Complex strategic initiative" for i in range(10)
        ]
