...
from supervisor_agent.plugins.plugin_interface import (
    PluginInterface, PluginMetadata, PluginEvent, PluginResult,
    PluginType, PluginStatus, EventType, PluginValidator,
    PluginPermissionManager, TaskProcessorInterface,
    DataSourceInterface, NotificationInterface, AnalyticsInterface,
...
class PluginDependencyResolver:
        ...
        for plugin_name, metadata in plugins.items():
            for dependency in metadata.dependencies:
                if dependency in graph:
                    ...
                    logger.warning(f"Plugin {plugin_name} has unresolved dependency: {dependency}")
    ...
    @staticmethod
    def validate_dependencies(plugins: Dict[str, PluginMetadata]) -> Dict[str, List[str]]:
...
class PluginEventBus:
    ...
    async def publish(self, event: PluginEvent):
        ...
        try:
            ...
            for weak_handler in subscribers[:]:  # Copy to avoid modification during iteration
            ...
            if delivery_tasks:
                ...
                self.metrics["delivery_failures"] += len(results) - successful_deliveries
            
            logger.debug(f"Published event {event.event_type} to {len(delivery_tasks)} subscribers")
...
class PluginManager:
    ...
    def __init__(self, plugin_directories: List[str] = None, config_file: str = None):
        self.plugin_directories = plugin_directories or ["supervisor_agent/plugins/enabled"]
    ...
    def _load_configurations(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                logger.info(f"Loaded configurations for {len(self.configurations)} plugins")
            else:
    ...
    async def load_plugin(self, plugin_name: str, plugin_path: str) -> bool:
        ...
        try:
            ...
            # Validate plugin
            validation_issues = PluginValidator.validate_interface_compliance(plugin_instance)
            if validation_issues:
                raise ValueError(f"Plugin validation failed: {validation_issues}")
            
            metadata_issues = PluginValidator.validate_metadata(plugin_instance.metadata)
            if metadata_issues:
            ...
            for permission in plugin_instance.metadata.permissions:
                self.permission_manager.grant_permission(plugin_instance.plugin_id, permission)
        ...
        try:
            ...
            for permission in registry_entry.metadata.permissions:
                self.permission_manager.revoke_permission(
                    registry_entry.plugin.plugin_id, 
                    permission
                )
    ...
    def list_plugins(self) -> Dict[str, Dict[str, Any]]:
        ...
        for plugin_name, registry_entry in self.registry.items():
            result[plugin_name] = {
                ...
                "last_activity": registry_entry.last_activity.isoformat() if registry_entry.last_activity else None,
    ...
    def get_plugins_by_type(self, plugin_type: PluginType) -> List[str]:
        ...
        return [
            name for name, registry_entry in self.registry.items()
