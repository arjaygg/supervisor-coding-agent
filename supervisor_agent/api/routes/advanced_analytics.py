...
from supervisor_agent.core.advanced_analytics_engine import (
    MetricsCollector, StatisticalAnomalyDetector, TimeSeriesPredictor,
    TimeSeries, MetricType, AnomalyType, AlertSeverity
)
...
class AnalyticsWebSocketManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.info(f"Analytics WebSocket connected. Total connections: {len(self.active_connections)}")
    
    def disconnect(self, websocket: WebSocket):
        self.active_connections.remove(websocket)
        self.subscriber_preferences.pop(websocket, None)
        logger.info(f"Analytics WebSocket disconnected. Total connections: {len(self.active_connections)}")
    ...
    def _filter_metrics_by_preferences(self, 
        ...
        while True:
            ...
            try:
                ...
                if message.get("type") == "update_preferences":
                    ...
                    logger.info(f"Updated WebSocket preferences: {message.get('preferences')}")
    ...
    try:
            ...
            try:
                ...
                for metric_name, value in metrics.items():
                    if isinstance(value, (int, float)) and metric_name != 'timestamp':
                        ...
                        ts = TimeSeries(metric_name=metric_name, metric_type=MetricType.GAUGE)
                        ts.add_point(datetime.now(timezone.utc), float(value))
                        
                        # For demo, add some historical points
                        for i in range(1, 10):
                            past_time = datetime.now(timezone.utc) - timedelta(minutes=i)
                            past_value = float(value) * (1 + (i * 0.01))  # Simple trend
                            ts.add_point(past_time, past_value)
                        
                        pred_list = predictor.predict(ts, forecast_horizon=6)  # 6 time periods ahead
                        if pred_list:
        ...
        if include_anomalies and metrics:
            try:
                ...
                critical_anomalies = [
                    anomaly for anomaly_list in anomalies.values()
...
async def get_dashboard_data(
    ...
    metric_types: List[str] = Query(["system", "tasks"], description="Metric types to include"),
    ...
    try:
        ...
        dashboard_data["summary"] = {
            "total_metrics_collected": len([
                metric for category in dashboard_data["metrics"].values()
...
async def get_analytics_insights(
    ...
    try:
        ...
        insights.sort(key=lambda x: (x.get("confidence", 0), x.get("impact_score", 0)), reverse=True)
...
async def _generate_trend_analysis(metrics_data: Dict[str, Any], time_range: str) -> Dict[str, Any]:
...
async def _generate_period_comparisons(metrics_data: Dict[str, Any], time_range: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            metadata_df = pd.DataFrame([
                ...
                ["Metric Types", ", ".join(data["export_metadata"]["metric_types"])],
                ...
                for metric_name, value in metrics.items():
                    if metric_name != 'timestamp':
                        rows.append({
                            "metric": metric_name,
                            "value": value,
                            "timestamp": metrics.get('timestamp', data["export_metadata"]["exported_at"])
                        })
