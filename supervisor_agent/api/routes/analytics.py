...
async def get_cost_summary(
    ...
    days: Optional[int] = Query(30, description="Number of days to analyze (if start_date not provided)"),
...
@router.get("/analytics/costs/entries", response_model=List[schemas.CostTrackingEntryResponse])
async def get_cost_entries(
    ...
    try:
        ...
        return [schemas.CostTrackingEntryResponse.model_validate(entry) for entry in entries]
...
@router.get("/analytics/usage/metrics", response_model=List[schemas.UsageMetricsResponse])
async def get_usage_metrics(
    metric_type: Optional[str] = Query(None, description="Filter by metric type (daily, hourly, agent)"),
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=1000),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        return [schemas.UsageMetricsResponse.model_validate(metric) for metric in metrics]
...
async def get_usage_trends(
    ...
    try:
        ...
        metrics = crud.UsageMetricsCRUD.get_metrics(db, metric_type=metric_type, limit=days * 2)
            ...
            try:
                ...
                if start_date <= metric_date <= end_date:
                    ...
                    trends["success_rates"].append({
                        "date": metric.metric_key,
                        "value": float(metric.success_rate)
                    })
...
async def get_agent_performance(
    ...
    try:
        ...
        # Get agent metrics
        agent_metrics = crud.UsageMetricsCRUD.get_metrics(db, metric_type="agent", limit=1000)
        ...
        for metric in agent_metrics:
            ...
            if recent_entries:
                ...
                avg_response_time = sum(entry.execution_time_ms for entry in recent_entries) / len(recent_entries)
                
                # Calculate efficiency metrics
                cost_per_token = total_cost / total_tokens if total_tokens > 0 else 0
                tokens_per_request = total_tokens / len(recent_entries) if recent_entries else 0
                ...
                agent_performance.append({
                    ...
                    "efficiency_score": float(metric.success_rate) / (cost_per_token * 1000 + 1)  # Higher is better
                })
        ...
        return {
            ...
            "summary": {
                ...
                "total_cost": sum(float(a["total_cost_usd"]) for a in agent_performance),
                "avg_success_rate": sum(a["success_rate"] for a in agent_performance) / len(agent_performance) if agent_performance else 0
            }
        }
...
async def get_cost_optimization_insights(
    ...
    try:
        ...
        expensive_requests = [e for e in recent_entries if float(e.estimated_cost_usd) > expensive_threshold]
        ...
        if expensive_requests:
            ...
            recommendations.append("Review high-cost requests for optimization opportunities")
        
        # Analyze token efficiency
        token_costs = [(e.total_tokens, float(e.estimated_cost_usd)) for e in recent_entries]
        if token_costs:
            avg_cost_per_token = sum(cost for _, cost in token_costs) / sum(tokens for tokens, _ in token_costs)
            ...
            inefficient_requests = [
                ...
                if e.total_tokens > 0 and (float(e.estimated_cost_usd) / e.total_tokens) > inefficient_threshold
            ]
        ...
        if len(model_usage) > 1:
            ...
            if most_expensive_model[1]["cost"] / total_cost > 0.6:  # More than 60% of cost
                insights.append({
                    "type": "model_usage",
                ...
                recommendations.append("Consider using more cost-effective models for simpler tasks")
        ...
        if hour_costs:
            ...
            if peak_hour[1] / total_cost > 0.2:  # More than 20% in single hour
                ...
                recommendations.append("Consider load balancing across different time periods")
...
async def estimate_task_cost(
    task_type: str,
    prompt_text: str,
    expected_response_length: Optional[int] = Query(None, description="Expected response length in characters"),
    model: Optional[str] = Query("claude-3-5-sonnet-20241022", description="Model to use for estimation")
):
    ...
    try:
        ...
        if expected_response_length:
            completion_tokens = TokenEstimator.estimate_tokens("x" * expected_response_length)
        else:
        ...
        estimated_cost = TokenEstimator.estimate_cost(prompt_tokens, completion_tokens, model)
        ...
        return {
            ...
            "estimated_execution_time_ms": 5000 + (prompt_tokens + completion_tokens) * 2  # Rough estimate
        }
