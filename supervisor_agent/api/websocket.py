...
class WebSocketManager:
    ...
    async def send_agent_status_update(self, agent_data: Dict[str, Any]):
        ...
        await self.broadcast(json.dumps(message))
        logger.debug(f"Sent agent status update for agent {agent_data.get('id', 'unknown')}")
    ...
    async def send_system_notification(self, notification: Dict[str, Any]):
        ...
        await self.broadcast(json.dumps(message))
        logger.info(f"Sent system notification: {notification.get('message', 'No message')}")
...
async def notify_system_event(event_type: str, message: str, details: Dict[str, Any] = None):
