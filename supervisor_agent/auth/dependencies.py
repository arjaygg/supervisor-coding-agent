...
from supervisor_agent.auth.models import User, UserSession, APIKey, SecurityAuditLog
from supervisor_agent.auth.schemas import UserResponse, TokenType
...
async def authenticate_api_key(request: Request, api_key: str, db: Session) -> User:
    ...
    if not jwt_handler.verify_api_key(api_key, api_key_record.hashed_key):
        log_security_event(
            db, "api_key_verification_failed",
    ...
    log_security_event(
        db, "api_key_authentication_success",
        user_id=user.id,
        details={"prefix": key_prefix},
        ip_address=request.client.host if request.client else None,
        user_agent=request.headers.get("user-agent"),
...
def require_permissions(permissions: Union[List[str], str]):
    ...
    def permission_dependency(
        ...
        if missing_permissions:
            ...
            raise AuthorizationError(f"Missing permissions: {', '.join(missing_permissions)}")
...
def require_user_or_admin(current_user: User = Depends(require_roles(["user", "admin"]))):
    return current_user
