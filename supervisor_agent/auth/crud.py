...
class RoleCRUD:
    ...
    @staticmethod
    def update_role(db: Session, role_id: str, role_update: RoleUpdate) -> Optional[Role]:
...
class SessionCRUD:
    ...
    @staticmethod
    def get_session_by_refresh_jti(db: Session, refresh_jti: str) -> Optional[UserSession]:
        return db.query(UserSession).filter(
...
def initialize_default_permissions(db: Session):
    ...
    for name, resource, action, description in default_permissions:
        if not PermissionCRUD.get_permission_by_name(db, name):
            PermissionCRUD.create_permission(db, PermissionCreate(
                name=name,
                resource=resource,
                action=action,
...
def initialize_default_roles(db: Session):
    ...
    user_permissions = [
        ...
        "analytics:read", "providers:read"
    ]
    
    # Viewer role (read-only)
    viewer_permissions = [
        "tasks:read", "workflows:read", "analytics:read", "providers:read"
    ]
    ...
    for name, description, permissions, is_system in roles_to_create:
        if not RoleCRUD.get_role_by_name(db, name):
            role = RoleCRUD.create_role(db, RoleCreate(
                name=name,
                description=description,
