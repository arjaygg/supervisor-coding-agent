...
class ProviderFactory:
    ...
    @classmethod
    def register_provider_class(cls, provider_type: ProviderType, provider_class: Type[AIProvider]):
        """Register a provider class for a specific type."""
        cls._provider_classes[provider_type] = provider_class
        logger.info(f"Registered provider class {provider_class.__name__} for type {provider_type}")
    ...
    async def create_provider(cls, config: ProviderConfig) -> AIProvider:
        ...
        logger.info(f"Created provider {config.provider_id} of type {config.provider_type}")
        return provider
...
class ProviderRegistry:
    ...
    def __init__(self, load_balancing_strategy: LoadBalancingStrategy = LoadBalancingStrategy.PRIORITY_BASED):
        ...
        logger.info(f"Provider Registry initialized with strategy: {load_balancing_strategy}")
    ...
    async def register_provider(self, config: ProviderConfig) -> None:
        ...
        try:
            ...
            raise ProviderError(f"Provider registration failed: {str(e)}", config.provider_id)
    ...
    async def unregister_provider(self, provider_id: str) -> None:
        ...
        try:
            ...
            logger.error(f"Error during provider unregistration {provider_id}: {str(e)}")
    ...
    async def execute_task(self, task: Task, provider_id: Optional[str] = None) -> ProviderResponse:
        """
        ...
        try:
            ...
            logger.info(f"Task {task.id} executed successfully by provider {provider_id}")
            return response
    ...
    async def start_health_monitoring(self, check_interval_seconds: int = 60):
        ...
        logger.info(f"Started health monitoring with {check_interval_seconds}s interval")
