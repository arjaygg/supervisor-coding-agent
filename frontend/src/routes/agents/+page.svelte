  ...
  {:else if agents.length === 0}
    <div class="bg-gray-800 rounded-lg p-8 text-center">
      ...
      <p class="text-gray-500">
        No Claude CLI agents are currently configured.
      </p>
    </div>
  {:else}
          ...
          <div class="space-y-3">
            <div>
              <div class="flex justify-between text-sm mb-1">
                ...
                <span class="text-gray-300"
                  >{agent.quota_used} / {agent.quota_limit}</span
                >
              </div>
              <div class="w-full bg-gray-700 rounded-full h-2">
                <div
                  class="h-2 rounded-full {getAgentStatus(agent) === 'error'
                    ? 'bg-red-500'
                    : getAgentStatus(agent) === 'warning'
                    ? 'bg-yellow-500'
                    : 'bg-green-500'}"
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span class="text-gray-300"
                  >{formatDate(agent.last_used_at)}</span
                >
              </div>
            </div>
            ...
            <div class="text-sm">
              ...
              <span class="text-gray-300"
                >{formatDate(agent.quota_reset_at)}</span
              >
            </div>

            <div class="flex gap-2 mt-4">
              <button
                class="btn-secondary text-xs px-3 py-1 {agent.is_active
              ...
              <button
                class="btn-outline text-xs px-3 py-1 {!agent.is_active
                  ? 'opacity-50 cursor-not-allowed'
                  : ''}"
                disabled={!agent.is_active}
              >
