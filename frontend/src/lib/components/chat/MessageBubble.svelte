...
<div class="flex {isUser ? 'justify-end' : 'justify-start'} space-x-3">
  ...
  {#if !isUser}
    <div class="flex-shrink-0">
      <div
        class="w-8 h-8 rounded-full flex items-center justify-center {isAssistant
          ? 'bg-blue-600'
          : 'bg-gray-600'}"
      >
        {#if isAssistant}
  ...
  <div class="flex-1 max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
    ...
    >
      ...
      {#if message.metadata && Object.keys(message.metadata).length > 0}
        <div
          class="mt-2 pt-2 border-t border-opacity-20 {isUser
            ? 'border-white'
            : 'border-gray-600'}"
        >
          {#if message.metadata.task_id}
            <div class="text-xs opacity-75">
              Related to task: <code class="bg-black bg-opacity-20 px-1 rounded"
                >{message.metadata.task_id}</code
              >
            </div>
          {/if}
    ...
    {#if showTimestamp}
      <div
        class="flex items-center space-x-2 mt-1 {isUser
          ? 'justify-end'
          : 'justify-start'}"
      >
