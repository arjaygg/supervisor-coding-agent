    ...
    {#if warnings.length > 0}
      <div class="mt-2 space-y-1">
        {#each warnings as warning}
          <div class="flex items-start space-x-2 text-xs p-2 rounded {
            warning.severity === 'info' ? 'bg-blue-900 bg-opacity-20 text-blue-300' :
            warning.severity === 'warning' ? 'bg-yellow-900 bg-opacity-20 text-yellow-300' :
            warning.severity === 'error' ? 'bg-red-900 bg-opacity-20 text-red-300' : ''
          }">
