    ...
    {:else}
      <div class="space-y-1 p-2">
        {#each filteredThreads as thread (thread.id)}
          <button
            class="w-full text-left p-3 rounded-lg transition-colors relative group {currentThreadId ===
            thread.id
            ...
            <div class="pr-8">
              <div class="flex items-center justify-between mb-1">
                ...
                {#if thread.unread_count && thread.unread_count > 0}
                  <span
                    class="bg-blue-500 text-white text-xs font-medium px-1.5 py-0.5 rounded-full min-w-[18px] text-center"
                  >
