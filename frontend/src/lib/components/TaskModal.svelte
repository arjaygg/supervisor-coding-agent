      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-700">
        <div>
          ...
          <p class="text-gray-400 text-sm mt-1">
            {task.type} • <span class="{getStatusColor(task.status)}">{task.status}</span>
          </p>
        </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
              <div class="bg-gray-900 rounded-lg p-4">
                <h3 class="text-lg font-medium text-white mb-4">Task Information</h3>
                
                <div class="space-y-3 text-sm">
                  <div class="flex justify-between">
                    <span class="text-gray-400">Created</span>
                    <span class="text-gray-300">{formatDate(task.created_at)}</span>
                  </div>
                  
                  <div class="flex justify-between">
                    <span class="text-gray-400">Updated</span>
                    <span class="text-gray-300">{formatDate(task.updated_at)}</span>
                  </div>
                  ...
                  {#if task.assigned_agent_id}
                    <div class="flex justify-between">
                      ...
                      <span class="text-gray-300 font-mono">{task.assigned_agent_id}</span>
                    </div>
                  {/if}
                  ...
                  {#if task.error_message}
                    <div class="mt-4">
                      ...
                      <div class="bg-red-900/20 border border-red-500/50 rounded p-3">
                        <code class="text-red-400 text-xs break-all">{task.error_message}</code>
                      </div>
                    </div>
                  {/if}
                </div>
              ...
              <!-- Task Payload -->
              <div class="bg-gray-900 rounded-lg p-4">
                <h3 class="text-lg font-medium text-white mb-4">Task Payload</h3>
                ...
                {#if editMode}
                  ...
                  ></textarea>
                {:else}
                  <pre class="bg-gray-800 rounded p-3 text-green-400 text-xs overflow-x-auto"><code>{JSON.stringify(task.payload, null, 2)}</code></pre>
                {/if}
              </div>
            ...
            <div class="space-y-6">
              <div class="bg-gray-900 rounded-lg p-4">
                <h3 class="text-lg font-medium text-white mb-4">Execution History</h3>
                
                {#if taskSessions.length === 0}
                  <p class="text-gray-500 text-center py-4">No execution history available</p>
                {:else}
                  <div class="space-y-4">
                    {#each taskSessions as session}
                      <div class="border border-gray-700 rounded-lg p-4">
                        <div class="flex justify-between items-center mb-2">
                          <span class="text-gray-400 text-sm">Session #{session.id}</span>
                          <span class="text-gray-400 text-sm">{formatDate(session.created_at)}</span>
                        </div>
                        ...
                        {#if session.prompt}
                          <details class="mb-2">
                            <summary class="text-blue-400 cursor-pointer text-sm">View Prompt</summary>
                            <pre class="mt-2 bg-gray-800 rounded p-2 text-xs text-gray-300 overflow-x-auto"><code>{session.prompt}</code></pre>
                          </details>
                        {/if}
                        
                        {#if session.response}
                          <details>
                            <summary class="text-green-400 cursor-pointer text-sm">View Response</summary>
                            <pre class="mt-2 bg-gray-800 rounded p-2 text-xs text-gray-300 overflow-x-auto"><code>{session.response}</code></pre>
                          </details>
                        {/if}
                      </div>
                    {/each}
