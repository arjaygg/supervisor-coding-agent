...
class GitHubPRMonitor:
    ...
    def run_command(
        self, cmd: str, cwd: Optional[Path] = None
    ...
    def get_open_prs(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.run_command(
                "gh pr list --state open --json number,title,headRefName,url,createdAt,author"
            )
    ...
    def check_pr_status(self, pr_number: int) -> Dict[str, Any]:
        ...
        try:
            # Get PR details
            result = self.run_command(
                f"gh pr view {pr_number} --json state,mergeable,reviewDecision,statusCheckRollup"
            )
    ...
    def _run_code_analysis(self) -> Dict[str, Any]:
        ...
        # Flake8
        result = self.run_command(
            f"{activate_cmd}flake8 supervisor_agent/ --format=json --output-file=flake8-results.json"
        )
        ...
        # Pylint
        result = self.run_command(
            f"{activate_cmd}pylint supervisor_agent/ --output-format=json > pylint-results.json"
        )
        ...
        # MyPy
        result = self.run_command(
            f"{activate_cmd}mypy supervisor_agent/ --config-file=mypy.ini --json-report mypy-report"
        )
        ...
        # Bandit
        result = self.run_command(
            f"{activate_cmd}bandit -r supervisor_agent/ -f json -o bandit-results.json"
        )
        ...
        # Semgrep
        result = self.run_command(
            f"semgrep --config=auto supervisor_agent/ --json --output=semgrep-results.json"
        )
        ...
        result = self.run_command(
            f"{activate_cmd}pytest supervisor_agent/tests/ -v --cov=supervisor_agent --cov-report=json --cov-report=html --junitxml=test-results.xml"
        )
        ...
        # Black
        result = self.run_command(
            f"{activate_cmd}black --check --diff supervisor_agent/"
        )
        ...
        # isort
        result = self.run_command(
            f"{activate_cmd}isort --check-only --diff supervisor_agent/"
        )
    ...
    def _post_review_comment(self, pr_number: int, results: Dict[str, Any]) -> None:
        ...
        critical_issues = self._count_critical_issues(results)
        status = (
            "✅ APPROVED" if critical_issues["critical"] == 0 else "❌ CHANGES REQUIRED"
        )
...
"""
        ...
        # Test results
        test_status = (
            "✅" if results["test_results"]["pytest"]["exit_code"] == 0 else "❌"
        )
        comment += f"\n#### Tests\n- {test_status} **pytest**\n"
    ...
    def _count_critical_issues(self, results: Dict[str, Any]) -> Dict[str, int]:
        ...
        return {"critical": critical, "warnings": warnings, "security": security}
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if coverage < 80:
            recommendations.append(
                f"📊 Improve test coverage (current: {coverage}%, target: 80%+)"
            )
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if result.returncode == 0:
                    ...
                    for run in runs:
                        if "Deploy Apps" in run.get("name", "") and run.get(
                            "status"
                        ) in ["completed"]:
                            if run.get("conclusion") == "success":
                                logger.info(
                                    f"Deployment successful for PR #{pr_number}"
                                )
                                return True
                            else:
    ...
    def run_post_deploy_tests(self, pr_number: int) -> bool:
        ...
        try:
            # Run smoke tests
            result = self.run_command(
                "python3 tests/e2e_smoke_tests.py --env dev --critical-only"
            )
    ...
    def merge_pr(self, pr_number: int) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Post merge comment
                self.run_command(
                    f'gh pr comment {pr_number} --body "🎉 **PR Merged Successfully**\\n\\nAutomated workflow completed:\\n- ✅ Code review passed\\n- ✅ Deployment successful\\n- ✅ Tests passed\\n- ✅ Merged to main"'
                )
                return True
            else:
            ...
            try:
                ...
                if not open_prs:
                    ...
                    for pr in open_prs:
                        ...
                        if success:
                            logger.info(
                                f"PR #{pr_number} workflow completed successfully"
                            )
                        else:
                            logger.warning(
                                f"PR #{pr_number} workflow failed or incomplete"
                            )
