...
class BranchCleanupManager:
    ...
    def get_merged_branches(self) -> List[str]:
        ...
        if not success:
            ...
            success, output, _ = self.run_command(['git', 'branch', '--merged', 'main-updated'])
        ...
        for line in output.split('\n'):
            ...
            if branch and branch not in self.main_branches:
                ...
                if not any(pattern in branch for pattern in ['main', 'master', 'HEAD', 'develop']):
    ...
    def get_remote_merged_branches(self) -> List[str]:
        ...
        success, output, _ = self.run_command(['git', 'branch', '-r', '--merged', 'origin/main'])
        if not success:
            return []
        ...
        for line in output.split('\n'):
            branch = line.strip()
            if branch and 'origin/' in branch and not any(main in branch for main in self.main_branches):
    ...
    def delete_remote_branch(self, branch: str) -> bool:
        ...
        success, _, error = self.run_command(['git', 'push', 'origin', '--delete', branch])
        if success:
    ...
    def cleanup_specific_branch(self, branch_name: str) -> bool:
        ...
        local_success, output, _ = self.run_command(['git', 'branch', '--list', branch_name])
        if local_success and output.strip():
            if not self.delete_local_branch(branch_name):
                success = False
        
        # Check if branch exists remotely
        remote_success, output, _ = self.run_command(['git', 'branch', '-r', '--list', f'origin/{branch_name}'])
...
def main():
    ...
    parser.add_argument(
        "--force", 
        action="store_true", 
        help="Delete branches without confirmation"
    )
    parser.add_argument(
        "--branch",
        help="Clean up a specific branch"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
