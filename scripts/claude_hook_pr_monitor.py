...
class ClaudeHookPRMonitor:
    ...
    def is_pr_branch(self) -> bool:
        ...
        return (
            any(branch.startswith(prefix) for prefix in pr_prefixes) or branch != "main"
        )
    ...
    def _analyze_typescript_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if eslint_result.returncode != 0:
                ...
                eslint_fix_result = self.run_command(
                    f"npx eslint --fix-dry-run {file_path}"
                )
...
Generated at {datetime.now().isoformat()}"""
    ...
    def handle_file_change(self, file_path: str, tool: str, action: str) -> None:
        ...
        logger.info(
            f"Processing file change: {file_path} (tool: {tool}, action: {action})"
        )
        ...
        if analysis_result.get("fixes"):
            ...
            if fixes_applied:
                ...
                logger.info(
                    f"✅ Processed {file_path}: {issue_count} issues, {fix_count} fixes applied"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--session-complete", action="store_true", help="Session completion hook"
    )
    parser.add_argument(
