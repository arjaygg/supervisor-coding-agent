...
logger = setup_script_logging("pr_reviewer_merger", "pr_reviewer_merger.log")
...
class PRReviewerAndMerger:
    ...
    def analyze_pr_status(self, pr: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if analysis["mergeable"] != "MERGEABLE":
            analysis["blocking_issues"].append(
                f"PR is not mergeable: {analysis['mergeable']}"
            )
            analysis["issues"].append(
                {
                    "type": "merge_conflict",
        ...
        analysis["ready_to_merge"] = (
            len(failed_checks) == 0
            and len(pending_checks) == 0
            and analysis["mergeable"] == "MERGEABLE"
            and not analysis["blocking_issues"]
        )

        logger.info(
            f"PR #{pr_number} analysis: {len(failed_checks)} failed checks, "
    ...
    def checkout_pr_branch(self, pr_number: int) -> bool:
        ...
        try:
            ...
            if "already used by worktree" in result.stderr:
                logger.info(
                    f"Worktree conflict detected for PR #{pr_number}, attempting resolution..."
                )

                # Get PR branch name
                pr_info_result = self.run_command(
                    f"gh pr view {pr_number} --json headRefName"
                )
                ...
                logger.info(
                    f"Trying manual fetch and checkout for branch {branch_name}"
                )

                # Fetch the branch
                fetch_result = self.run_command(
                    f"git fetch origin {branch_name}:{branch_name}-local"
                )
                ...
                if checkout_result.returncode != 0:
                    ...
                    if checkout_result.returncode != 0:
                        ...
                        delete_result = self.run_command(
                            f"git branch -D {branch_name}-local"
                        )
                        # Create local branch tracking the remote
                        checkout_result = self.run_command(
                            f"git checkout -b {branch_name}-local origin/{branch_name}"
                        )
                        if checkout_result.returncode != 0:
                            logger.error(
                                f"Failed to create local branch for {branch_name}"
                            )
                            return False
    ...
    def fix_commit_messages(self, pr_number: int) -> bool:
        ...
        try:
            ...
            commit_msg = (
                last_commit.split(" ", 1)[1] if " " in last_commit else last_commit
            )
            ...
            if not commit_msg.startswith(
                ...
                if any("test" in f for f in changed_files):
                    ...
                    "fix" in commit_msg.lower() or "bug" in commit_msg.lower()
                    for f in [commit_msg]
                ):
                    commit_type = "fix"
                ...
                if not self.dry_run:
                    ...
                    result = self.run_command(
                        f'git commit --amend -m "{new_commit_msg}"'
                    )
                    if result.returncode == 0:
...
Generated at {datetime.now().isoformat()}"""
                ...
                logger.info(
                    f"Would commit and push fixes for PR #{pr_number}: {fixed_files}"
                )
                return True
...
The PR checks should re-run automatically. Please review the changes."""
                    ...
                    if not self.dry_run:
                        self.run_command(
                            f'gh pr comment {pr_number} --body "{comment}"'
                        )
    ...
    def approve_pr(self, pr_number: int) -> bool:
        ...
        try:
            if not self.dry_run:
                result = self.run_command(
                    f'gh pr review {pr_number} --approve --body "✅ **Automated Review - APPROVED**\\n\\nAll checks passed. This PR is ready for merge."'
                )
                if result.returncode == 0:
    ...
    def merge_pr(self, pr_number: int) -> bool:
        ...
        try:
            ...
            # Final safety check
            result = self.run_command(
                f"gh pr view {pr_number} --json mergeable,reviewDecision"
            )
            if result.returncode != 0:
                return False
    ...
    def process_pr(self, pr: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if analysis["fixes_available"] and any(
                f.get("auto_fixable") for f in analysis["fixes_available"]
            ):
                ...
                if self.apply_fixes_to_pr(analysis):
                    ...
                    if self.wait_for_checks_completion(pr_number):
                        ...
                        logger.warning(
                            f"Checks did not complete in time for PR #{pr_number}"
                        )
                else:
                    logger.warning(f"Failed to apply fixes to PR #{pr_number}")
            else:
                logger.info(
                    f"PR #{pr_number} has no auto-fixable issues or requires manual review"
                )

                # Add comment about issues found
                if analysis["issues"]:
                    issues_summary = "\n".join(
                        f"- {issue['message']}" for issue in analysis["issues"]
                    )
        ...
        if continuous:
            try:
                while True:
                    ...
                    logger.info(
                        f"Waiting {self.check_interval} seconds before next cycle..."
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced PR Review and Auto-Merge System"
    )
    parser.add_argument(
        "--continuous", action="store_true", help="Run continuously (for daemon mode)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-auto-fix", action="store_true", help="Disable automatic fixes"
    )
    parser.add_argument(
        "--no-auto-merge", action="store_true", help="Disable automatic merging"
    )
