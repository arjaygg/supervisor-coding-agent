...
class DeploymentValidator:
    ...
    async def validate_deployment(self, critical_only: bool = False) -> Dict[str, Any]:
        ...
        self.logger.info(f"🚀 Starting post-deployment validation for {self.environment}")
    ...
    async def _validate_api_endpoints(self):
        ...
        endpoints_to_test = [
            ...
            ("/api/v1/analytics/providers/dashboard", "GET", False),  # Multi-provider endpoint
        ]
                ...
                try:
                    ...
                    if method == "GET":
                        async with session.get(url) as response:
                            success = response.status in [200, 400, 401]  # 400/401 are acceptable for some endpoints
                            ...
                            if not success and is_critical:
                                raise Exception(f"Critical endpoint {endpoint} returned {response.status}")
    ...
    async def _validate_database(self):
        ...
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            ...
            try:
                async with session.get(f"{self.base_url}/api/v1/analytics/summary") as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        if response.status >= 500:
                            raise Exception(f"Analytics endpoint returned {response.status} - likely database issue")
                        else:
                            self.results.append({
                                "check": "Database Connectivity",
                                "status": "warning",
                                "critical": True,
                                "details": {"status_code": response.status, "note": "Non-500 error suggests DB is working"}
                            })
    ...
    async def _validate_websockets(self):
        ...
        try:
            ...
            async with websockets.connect(f"{ws_url}/ws", ping_timeout=10) as websocket:
                # Send a test message
                test_message = json.dumps({"type": "ping", "timestamp": datetime.utcnow().isoformat()})
            ...
            try:
                async with websockets.connect(f"{ws_url}/ws/providers", ping_timeout=10) as websocket:
                ...
                self.results.append({
                    ...
                    "status": "skip",
                    "critical": False,
                    "details": {"note": "Provider WebSocket not available (multi-provider may be disabled)"}
                })
    ...
    async def _validate_multi_provider(self):
        ...
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            ...
            try:
                async with session.get(f"{self.base_url}/api/v1/analytics/providers/dashboard") as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        self.results.append({
                            ...
                            "details": {
                                ...
                                "total_providers": data.get("overview", {}).get("total_providers", 0),
                                "healthy_providers": data.get("overview", {}).get("healthy_providers", 0)
                            }
                        })
            ...
            async with session.get(f"{self.base_url}/") as response:
                ...
                self.results.append({
                    ...
                    "details": {
                        ...
                        "missing_security_headers": [h for h in security_checks.keys() if h not in headers]
                    }
                })
            ...
            try:
                ...
                async with session.get(f"{self.base_url}/api/v1/analytics/summary") as response:
                    if response.status == 200:
                        ...
                        raise Exception(f"Configuration check failed with status {response.status}")
    ...
    def _generate_validation_report(self) -> Dict[str, Any]:
        ...
        failed_checks = sum(1 for r in self.results if r.get("status") in ["fail", "error"])
        warning_checks = sum(1 for r in self.results if r.get("status") in ["warning", "slow"])
        
        critical_checks = [r for r in self.results if r.get("critical", False)]
        critical_failures = sum(1 for r in critical_checks if r.get("status") in ["fail", "error"])
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        critical_failures = [r for r in self.results if r.get("critical") and r.get("status") in ["fail", "error"]]
        if critical_failures:
            recommendations.append("🔥 Address critical failures before proceeding with deployment")
            
        failed_checks = [r for r in self.results if r.get("status") in ["fail", "error"]]
        if failed_checks:
        ...
        if slow_responses:
            recommendations.append("⚡ Investigate performance issues for slow-responding endpoints")
            
        security_issues = [r for r in self.results if "Security" in r.get("check", "") and r.get("status") != "pass"]
        if security_issues:
            recommendations.append("🔒 Review and implement missing security headers")
            
        if not recommendations:
            recommendations.append("✅ All validations passed successfully - deployment looks good!")
...
def print_validation_report(report: Dict[str, Any]):
    ...
    for check in report['checks']:
        ...
        print(f"   {critical_marker} {status_icon} {check.get('check', 'Unknown Check')}")
...
async def main():
    """Main CLI interface"""
    parser = argparse.ArgumentParser(description="Post-deployment validation for multi-provider architecture")
    parser.add_argument("--url", required=True, help="Base URL of deployed application")
    parser.add_argument("--env", required=True, choices=["staging", "production", "testing"], help="Environment name")
    parser.add_argument("--critical-only", action="store_true", help="Run only critical validations")
