...
class EnvironmentStateManager:
    ...
    def cleanup_ephemeral(self, max_age_hours: int = 24):
        ...
        for env_name, env_data in list(state.items()):
            if env_data.get('environment_type') == EnvironmentType.EPHEMERAL.value:
                ...
                age_hours = (now - deployment_time.replace(tzinfo=None)).total_seconds() / 3600
    ...
    def get_history(self, environment: Optional[str] = None, limit: int = 10) -> List[Dict]:
...
def main():
    ...
    update_parser.add_argument('--from-env', action='store_true',
                              help='Create deployment from environment variables')
    update_parser.add_argument('--environment', help='Environment name')
    update_parser.add_argument('--status', choices=[s.value for s in DeploymentStatus],
    ...
    list_parser.add_argument('--format', choices=['json', 'table', 'summary'],
                            default='table', help='Output format')
    
    # Cleanup command
    cleanup_parser = subparsers.add_parser('cleanup', help='Clean up old ephemeral environments')
    ...
    export_parser.add_argument('--format', choices=['json', 'summary'],
    ...
    if args.command == 'update':
        if args.from_env:
            ...
            if not args.environment:
                print("Error: --environment required when not using --from-env", file=sys.stderr)
                sys.exit(1)
            ...
            deployment = DeploymentState(
                ...
                commit_sha='unknown',
                deployer='manual',
                status=DeploymentStatus(args.status) if args.status else DeploymentStatus.DEPLOYED,
                timestamp=datetime.datetime.now().isoformat()
            )
        ...
        if args.format == 'json':
            print(json.dumps([asdict(env) for env in environments], indent=2, default=str))
        elif args.format == 'summary':
            for env in environments:
                pr_info = f"PR#{env.pr_number}" if env.pr_number else "N/A"
                print(f"{env.environment}: {env.status.value} ({pr_info}, {env.branch}, {env.deployer})")
        else:  # table format
            if not environments:
                print("No environments found")
                return
            
            print(f"{'Environment':<20} {'Type':<12} {'Status':<12} {'PR':<8} {'Branch':<20} {'Deployer':<15} {'Timestamp'}")
            print("-" * 110)
            for env in environments:
                pr_info = str(env.pr_number) if env.pr_number else "N/A"
                timestamp = env.timestamp[:16] if len(env.timestamp) > 16 else env.timestamp
                print(f"{env.environment:<20} {env.environment_type.value:<12} {env.status.value:<12} "
        ...
        print(f"{'Timestamp':<20} {'Environment':<20} {'Status':<12} {'PR':<8} {'Deployer':<15}")
        print("-" * 85)
        for entry in history:
            timestamp = entry['timestamp'][:16] if len(entry['timestamp']) > 16 else entry['timestamp']
            pr_info = str(entry.get('pr_number', 'N/A'))
            print(f"{timestamp:<20} {entry['environment']:<20} {entry['status']:<12} "
